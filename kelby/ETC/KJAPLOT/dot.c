...
/*    bufstat=0 nothing done yet 
      ...
      static int isg=0;
      char xtitle[20][40],ytitle[20][40],mtitle[80];
...
/*          fp_dot=fopen("dot.scratch","w"); */ /* write open */
          fp_dot=fopen("dot.scratch","w+b");  /* write open */
/*          fp_diag=fopen("dot.diag","w"); */ 
          bufstat=1;}
        if(k<0 || k>19) goto s80;
        dot.tsum[k]=0.0;
        dot.sum1[k]=0.0;
        dot.sum2[k]=0.0;
        dot.x2sum1[k]=0.0;
...
/*      finish up            */
  ...
  dotdpt(k)    
   ...
   { double dx,dy,dely,yy,dl,yup,ydn,xx,xxx;
     ...
     int iword,ipnt,itest,l1,l2,nwds,ierr,count;
      count=0;
...
loop: 
     for(i=0;i<sct.nsbf;i++)
	  ...
	  count++;
          pnta(kx,ky);}
      if(ntotal == 0 || sct.nrec == ntotal) 
       { /* fprintf(fp_diag," plot=%d  count=%d  nsbf=%d  nrec=%d\n",k,count,sct.nsbf,sct.nrec); */
         return(0);
       }
      nwds=fread(&sct.nsbf,4,2502,fp_dot);
      ierr=ferror(fp_dot);
