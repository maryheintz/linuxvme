...
chk_trig()
  { double asum,axsum,axxsum,ped,dped,sig2,aval,yc;
    ...
    double siglp,siglc,pmaxl,cmaxl,tmaxl;
    int nevnt,nevmax,nstrtev,l,nn,i,k,wait;
    int ipeak,timc,timf,kipk,ibt,kbt;
    int dac,time1,time2,time3,time4;
    double fdac,charge,val,ratio,fbt;
    int bigain,kpeak,klo,khi,icnt,icnt1,icnt2,dstep;
    double s,sx,sy,sxy,sev,sxev,sxxev,sxest,el,er,ep,estimator;
    double sig,sx2,volt,capacitor,a,b,del,x,y,dely;
    double pedlo,dpedlo;
    ...
    tmaxl=0.0;
        ...
        dotset(2,240.0,0.0,1600.0,1100.0,0,0);
        sprintf(xstring,"fine time setting");
        ...
        dotset(10,800.0,0.0,20.0,0.0,0,0);
        sprintf(xstring,"charge");
        sprintf(ystring,"bits");
        dotlbl(10,xstring,ystring);
...
/*     display ? */
	 ...
	 for(ibt=0;ibt<12;ibt++)
	   { kbt = (fdata[0][kpeak]>>ibt) & 1;
	     if(kbt == 1)
	       { fbt = (double)ibt + 0.5;
	         hstacc(3,fbt,1.0);
	         dotacc(10,charge,fbt);
	       }
	   }
      } /* end of nevnt loop */
        val=0.0;
...
/*-------------- test the trigger output switch -------------- */
   ...
   slope = b;
   kFAIL=0;
   if(slope<2.7) kFAIL=1;
   if(slope>2.9) kFAIL=2;
