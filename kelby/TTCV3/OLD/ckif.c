...
/*  label vertical axis  */
	...
	/*     BPutLong(1,ttcadr,1,0xc0,0x00); */ /* load set tp_high into fifo1 */
     ...
     inject()
       { undigned short *adr;
         (unsigned short *)adr = (vme24base + 0xde0000);
	 *adr = 0;
       }
...
clear_buffer()
    { unsigned long word,end,strt;
      strt=0;
loop:
      pio_rdwd(word,strt,end);
      if(end |= 1) goto loop;
    }
pio_read()
        {
...
pio_rdwd(unsigned long *word,int strt,int *end)
  { unsigned long wd1,wd2,ctrl,flag,*adr,pio_base;
    pio_base = vme24base + 0x570000;
    adr = (unsigned long *)(pio_base + 0x18);
loop1:
    wd1 = *adr;
    ctrl = (wd1>>17) & 1;
    flag = (wd1>>16) & 1;
    if((strt == 1) && (ctrl != 1) && (flag != 0)) goto loop1;
    wd2 = *adr;
    *word = wd1<<16 + (wd2&0xffff);
    *end = 0;
    if(ctrl==1 && flag==1) *end = 1; 
  }
