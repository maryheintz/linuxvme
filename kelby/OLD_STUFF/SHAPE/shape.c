	...
	for(m=0;m<4;m++)
	  { for(i=0;i<2650;i++)
	    {avg[m][i]=avg[m][i]/(double)event;
	     if(m==0 && avg[m][i]>fmax) fmax=avg[m][i];
	    }
	  }
         n1=ksave/250;
	 n1=n1*250;
	 printf("scale=%f  fmax=%f  n1=%d\n",scale,fmax,n1);
         sprintf(string,"large capacitor   charge=%5.1f  event=%d",
	   charge,event);
         onescale(250,string,n1,0);
	 if(fpsw(2)==0) goto loop;
