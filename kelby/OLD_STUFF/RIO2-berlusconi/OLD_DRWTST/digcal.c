...
int digadr[9] = {0,0x100e,0x100f,0x176e,0x1011,
...
unsigned int deskew1[9] = {0,0,0,0,0,0,0,0,0};
unsigned int deskew2[9] = {100,100,130,100,130,100,52,100,100};
...
slink_read()
	...
	int parity,samples,error,sestr,destr,r_parity,v_parity,
	   mode,gain,bunch;
	int type_of_word,count,kk,ii,block;
...
/* ====================== read out the 16 tileDMU chips ============== */
	...
	gain = (word>>12) & 0x7;
	bunch = word & 0xfff;
	printf("%d block=%d logain header word %x   bunch %d  %x %x\n"
...
/*	printf(" parity=%d  samples=%d  error=%d\n",
	...
	gain = (word>>12) & 0x7;
	bunch = word & 0xfff;
	printf("%d  %d higain header word=%x   bunch=%d  %x %x\n",
	   count,block,word,bunch,crca,crcb);
/*	printf(" parity=%d  samples=%d  error=%d\n",
	   parity,samples,error);
	printf(" SEstr=%d   r_parity=%d   v_parity=%d\n",
	...
	for(ii=0;ii<nsamp+1;ii++)
	    ...
	    if(code != SLINK_OK) goto err;
	    if (type_of_word == SLINK_CONTROL_WORD) goto err;  
	        printf("%d  digcrc word=%x   %x\n",count,word,code);
	    count++;
            code = SSP_ReadOneWord(dev, &word, &type_of_word);
	    if(code != SLINK_OK) goto err;
	    if (type_of_word == SLINK_CONTROL_WORD) goto err;   
	        printf("%d   global crc word=%x   %x\n",count,word,code);
            count++;
            code = SSP_ReadOneWord(dev, &word, &type_of_word);
	    if(code != SLINK_OK) goto err;
	    if (type_of_word != SLINK_CONTROL_WORD) goto err;   
	        printf(" end control word=%x   %x\n\n\n",word,code);
	return;
