...
loop:
	   ...
	   slink_read();
		  ...
		  goto skip;}
              if(sw(7) == 0)
   		{ if(icap == 0) sprintf(string,"LOW GAIN TUBES 1-24  BCID=%d",bcid);
  		  if(icap == 1) sprintf(string,"HIGH GAIN TUBES 1-24  BCID=%d",bcid);
  		  bigdisp(nsamp+1,string,1);
  		  if(icap == 0) sprintf(string,"LOW GAIN TUBES 25-48  BCID=%d",bcid);
  		  if(icap == 1) sprintf(string,"HIGH GAIN TUBES 25-48  BCID=%d",bcid);
