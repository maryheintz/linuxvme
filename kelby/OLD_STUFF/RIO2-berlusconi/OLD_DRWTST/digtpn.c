...
int dm[8] = {2,2,2,2,2,2,2,2};
int ddac1[8]={0,0,0,0,0,0,0,0};
int ddac2[8] ={0,0,0,0,0,0,0,0};
...
slink_read()
        {
	unsigned long word,wordlast,crc1,crc2,h1,h2,sw7;
	...
	{if (type_of_word != SLINK_CONTROL_WORD) 
		  ...
		  if(kklast == 15 && ii == 0) 
		   ...
		   { parity = (word>>30) & 1;
			...
			{printf("hcnt error count=%d\n",count);
		     ...
		     if(sw7 == 1) printf("%d header word %x  bunch %d  %d\n",
		   ...
		   { crc(1,word);
		     crcaa = crca & 0x7fff;
                     if(sw7 == 1) printf(" data word= %4i %4i %4x %4x %4x    %8x  %4x %4x\n",
		 ...
		 if(kk<8)
	       ...
	       { cc++;
	         if(sw7 == 1) printf("control word= %x\n", word);
...
/* set up digitizer registers */
	...
	SendLongCommand(adrttc,1,3,0xff);       /* set test part 1 LSB */	
	usleep(1);
        BroadcastShortCommand(0x40);            /* reset digitizer */

	SendLongCommand(adrttc,1,4,0xff);       /* set test part 2 */	
	usleep(1);
        BroadcastShortCommand(0x40);            /* reset digitizer */

	SendLongCommand(adrttc,1,5,0xff);       /* set test part 3 */	
	usleep(1); 
        BroadcastShortCommand(0x40);            /* reset digitizer */

	SendLongCommand(adrttc,1,6,0xff);       /* set test part 4 MSB */	
	usleep(1); 
        BroadcastShortCommand(0x40);            /* reset digitizer */
