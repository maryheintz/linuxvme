...
int ddac1[9]={90,90,90,90,120,190,190,90,90};
int ddac2[9] ={90,90,90,90,120,190,190,90,90};
...
int code;
static int nsamp;
int event=0,nerror=0,ndcrc=0,nbnch=0,ngcrc=0,nctrl=0,ndata=0;
...
/*    printf(" sending reset to interface\n"); */
    ...
    SetTriggerSelect(3);
    kkk =GetTriggerSelect(); 
    printf(" trigger active = %d\n",kkk);
    for(kk=0;kk<5;kk++)
...
loop:
       ...
       if((event%50) == 0) 
             printf("event=%d  nerror=%d  ndcrc=%d  nbnch=%d  nctrl=%d  ndata=%d\n",
               event,nerror,ndcrc,nbnch,nctrl,ndata);
       inject();
       usleep(1);
	   ...
	   slink_read();
		  ...
		  goto skip;}
              if(sw(7) == 0)
   		{ if(icap == 0) sprintf(string,"LOW GAIN TUBES 1-24  BCID=%d   k=%d",bcid,kkk);
  		  if(icap == 1) sprintf(string,"HIGH GAIN TUBES 1-24  BCID=%d   k=%d",bcid,kkk);
  		  bigdisp(nsamp+1,string,1);
  		  if(icap == 0) sprintf(string,"LOW GAIN TUBES 25-48  BCID=%d   k=%d",bcid,kkk);
  		  if(icap == 1) sprintf(string,"HIGH GAIN TUBES 25-48  BCID=%d   k=%d",bcid,kkk);
...
slink_read()
	...
	{if (type_of_word != SLINK_CONTROL_WORD) 
		 ...
		 if(kk<8)
	       ...
	       { cc++;
	         if(sw(8) == 0) printf("control word= %x  count=%d\n", word,count);
