...
#include "SLINK.h"
#include "ttcvi.h"
int fdlow[49][32],ipk[49],pk[49];
int fdhi[49][32],ipk[49],pk[49];
...
int ddac1[9]={90,90,90,90,90,90,90,90,90};
int ddac2[9] ={90,90,90,90,90,90,90,90,90};
...
char *envs[];
  { 
    ...
    int tim,i,l,k, kkk, drawer;
    int tube, card_num;
    int dac,tm,tm1,tm2,tm3,tm4;
    ...
    inject_setup();
    printf(" calling slink setup\n");
    slink_setup();
	
    printf(" calling digitizer setup\n");
    for(i=1;i<9;i++) digitizer_setup(i);  
    printf(" returned from digitizer setup\n");

    BroadcastShortCommand(3);     /* BunchCrossing and 
                                       event counter reset */
    usleep(100);
    printf("calling clear_buffer\n");
    clear_buffer();
    tm=0;
...
loop:  if(sw(5) == 1)
    ...
    SetTimeDes1(digadr[1],0);
    SetTimeDes2(digadr[1],100);
    CoarseDelay(digadr[1],0,0);

    SetTimeDes1(digadr[2],0);
    SetTimeDes2(digadr[2],130); 
    CoarseDelay(digadr[2],0,0);

    SetTimeDes1(digadr[3],0);
    SetTimeDes2(digadr[3],100);
    CoarseDelay(digadr[3],0,0);

    SetTimeDes1(digadr[4],0);
    SetTimeDes2(digadr[4],118);
    /* SetTimeDes2(digadr[4],tm); */
    CoarseDelay(digadr[4],0,0);

    SetTimeDes1(digadr[5],0);
    SetTimeDes2(digadr[5],82);
    CoarseDelay(digadr[5],0,0);

    SetTimeDes1(digadr[6],0);
    SetTimeDes2(digadr[6],13);
    CoarseDelay(digadr[6],0,0);

    SetTimeDes1(digadr[7],0);
    SetTimeDes2(digadr[7],56);
    CoarseDelay(digadr[7],0,0);

    SetTimeDes1(digadr[8],0);
    SetTimeDes2(digadr[8],40);
    CoarseDelay(digadr[8],0,0);
    printf(" ======================= tm=%d digadr=%x =============\n",tm,digadr[8]);
...
/*disp:	printf("To display data, please input channel num(1--48)\n");
	...
	if ( kk == 0) goto dispexit101; */
        asum = 0.0;
              ...
              axxsum = 0.0;
              for(i=1;i<3;i++)
               { if(icap == 0) aval = (double)fdlow[kk][i];
                 if(icap == 1) aval = (double)fdhi[kk][i];
                 asum = asum+1.0;
                 axsum = axsum + aval;
		...
		sidev(1);  /* force plot to screen without asking*/
               if(sw(6) == 0)
  		{ if(icap == 0) sprintf(string,
		  "LOW GAIN TUBES 1-24  tm3=%d tm4=%d",tm3,tm4);
  		  if(icap == 1) sprintf(string,"HIGH GAIN TUBES 1-24");
  		  bigdisp(nsamp+1,string,1);
		} 
  		if(icap == 0) sprintf(string,
		  "LOW GAIN TUBES 25-48  tm2=%d  tm1=%d",tm2,tm1);
...
evdisp(nn,string,kk)
 ...
 { int max,mmax,idx,idy,ny,linx,ixbas,iybas,ix,iy,iylst,kx,ky,i,k,ii,iii;
   ...
   if(icap == 0)
     { if (max < fdlow[kk][iii]) max = fdlow[kk][iii]; }
   if(icap == 1)
     { if (max < fdhi[kk][iii]) max = fdhi[kk][iii]; }
   };
   mmax=5000;
   ...
   iylst = 0;
   for(i=1;i<=nn;i++)
    { if(icap == 0) ii = fdlow[kk][i];
      if(icap == 1) ii = fdhi[kk][i];
      y = (double)ii;
      y = y*dy;
      iy = (int)y;
      idy = iy-iylst;
...
slink_read()
	...
	int parity,samples,error,sestr,destr,r_parity,v_parity,
	   mode,gain,bunch;
	int type_of_word,count,kk,ii,block,sw8;
...
/* ====================== read control word ========================== */
	...
	if (type_of_word == SLINK_CONTROL_WORD) 
	     if( sw8 == 0) printf(" control word=%x\n",word);
/* ====================== read out the 16 tileDMU chips ============== */
	...
	gain = (word>>12) & 0x7;
	bunch = word & 0xfff;
	if( sw8 == 0) printf("%d block=%d logain header word %x   bunch %d  %x %x\n"
	,count,block,word,bunch,crca,crcb);
/*	if( sw8 == 0) printf(" parity=%d  samples=%d  error=%d\n",
	   parity,samples,error);
	if( sw8 == 0) printf(" SEstr=%d   r_parity=%d   v_parity=%d\n",
	   sestr,r_parity,v_parity);
	if( sw8 == 0) printf(" mode=%d   gain=%d   bunch=%d\n",
	   mode,gain,bunch); */
	count++;
        /* ------------- samples low gain ---------------------------- */
	for(ii=0;ii<nsamp+1;ii++)
		...
		{ fdlow[3*kk+1][ii] = sd.bits.data1;
		  ...
		  fdlow[3*kk+3][ii] = sd.bits.data3;
                ...
                if( sw8 == 0) printf("%d  ldata word= %4i %4i %4x %4x %4x",count,
	           sd.bits.topbit,sd.bits.parity,
		   sd.bits.data1,sd.bits.data2,
		   sd.bits.data3);
		if( sw8 == 0) printf("   %d  %d    %x    %x %x\n",kk,ii,word,crca,crcb);
		count++;
	    }
	/* ------------- header high gain ---------------------------- */
        code = SSP_ReadOneWord(dev, &word, &type_of_word);
	...
	gain = (word>>12) & 0x7;
	bunch = word & 0xfff;
	if( sw8 == 0) printf("%d  %d higain header word=%x   bunch=%d  %x %x\n",
	   count,block,word,bunch,crca,crcb);
/*	if( sw8 == 0) printf(" parity=%d  samples=%d  error=%d\n",
	   parity,samples,error);
	if( sw8 == 0) printf(" SEstr=%d   r_parity=%d   v_parity=%d\n",
	...
	for(ii=0;ii<nsamp+1;ii++)
		...
		{ fdhi[3*kk+1][ii] = sd.bits.data1;
		  ...
		  fdhi[3*kk+3][ii] = sd.bits.data3;
                ...
                if( sw8 == 0) printf("%d  hdata word= %4i %4i %4x %4x %4x",count,
	           sd.bits.topbit,sd.bits.parity,
		   sd.bits.data1,sd.bits.data2,
		   sd.bits.data3);
		if( sw8 == 0) printf("   %d  %d    %x   %x %x\n",kk,ii,word,crca,crcb);
		count++;
            ...
            if(code != SLINK_OK) goto err;
 	    if (type_of_word == SLINK_CONTROL_WORD) goto err;
            if( sw8 == 0) printf("%d   crc=%x\n",count,word);
	    count++;
      } /* end kk loop */
            code = SSP_ReadOneWord(dev, &word, &type_of_word);
	    if(code != SLINK_OK) goto err;
	    if (type_of_word == SLINK_CONTROL_WORD) goto err;  
	        if( sw8 == 0) printf("%d  digcrc word=%x   %x\n",count,word,code);
	    count++;
            code = SSP_ReadOneWord(dev, &word, &type_of_word);
	    if(code != SLINK_OK) goto err;
	    if (type_of_word == SLINK_CONTROL_WORD) goto err;   
	        if( sw8 == 0) printf("%d   global crc word=%x   %x\n",count,word,code);
            count++;
            code = SSP_ReadOneWord(dev, &word, &type_of_word);
	    if(code != SLINK_OK) goto err;
	    if (type_of_word != SLINK_CONTROL_WORD) goto err;   
	        if( sw8 == 0) printf(" end control word=%x   %x\n\n\n",word,code);
	return;
err:    printf(" error return code=%x   word=%x\n",code,word);
       }
...
bigdisp(nn,string,strt)
 ...
 int max,mmax,idx,idy,ny,linx,ixbas,iybas,ix,iy,iylst,kx,ky,i,k,ii,iii;
   ...
   iylst = 0;
   for(i=0;i<=nn;i++)
    { if(icap == 0) ii = fdlow[kk+strt][i];
      if(icap == 1) ii = fdhi[kk+strt][i];
      y = (double)ii;
      y = y*dy;
      iy = (int)y;
      idy = iy-iylst;
