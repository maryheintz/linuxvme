...
main()
{ 
   int isw[6],i1,i2,ll,i,mode,s1,s2,s3,s4,itr,ire,mse,tpe,lcal,sw1,dacset;
   ...
   printf("  6 = coarse dac set loop\n");
   printf("  7 = fine dac set loop\n");
   ...
   printf(" 20 = coarse dac set loop intg on\n");
   printf(" 21 = set dac values\n");
   printf(" 22 = toggle coarse dac 127-255 fine=255\n");
   printf(" 23 = controller-3in1 read/write test aaa-555\n");
   ...
   if(mode == 3)
    {printf(" s1 s2 s3 s4 test using multi_sel instead of card_sel\n");
     loop3:
       ...
       card_sel(cadr);
       multi_high();
       send12_3in1(2,0,0x5);   /* set s1=s3=1 s2=s4=0 */
       send12_3in1(2,0,0xa);   /* set s1=s3=0 s2=s4=1 */
       send12_3in1(2,0,0xf);     /* set s1=s2=s3=s4=1   */
       send12_3in1(2,0,0);     /* set s1=s2=s3=s4=0   */
...
loop5:
   ...
   if(mode == 6)
    {loop6:
       for(i=0;i<256;i++)
         {send12_3in1(6,0,i);  /* set coarse dac value */
	  usec=20;
          usleep(usec);
         }   
       sleep(1);
   ...
   if(mode == 7)
    {loop7:
       for(i=0;i<256;i++)
         {send12_3in1(6,1,i);  /* set fine dac value */
	  usec=20;
          usleep(usec);
         }   
       sleep(1);
   ...
   if(mode == 8)
    {  send4_3in1(4,1);   /* enable calibration */
       ...
       send12_3in1(6,1,dval);  /* set fine dac value */
...
loop8:
       ...
       for(i=0;i<256;i++)
        {loops:
         dval = (unsigned short)i;
         send12_3in1(6,0,dval);  /* set coarse dac value */
         tp_high();
	 usec=10;
         usleep(usec);
         stime();
   ...
   if(mode == 9)
    {loop9:
       multi_low();
       printf("\ns1..4=0 0 0 0 itr=0 ire=0 mse=0 tpe=0 lcal=0 sent\n");
       ...
       tpe = (i>>10) & 1;
       lcal = (i>>11) & 1;
       printf("s1..4=%d %d %d %d itr=%d ire=%d mse=%d tpe=%d lcal=%d %x\n"
           ,s1,s2,s3,s4,itr,ire,mse,tpe,lcal,i);
       sleep(1);
       printf("\ns1..4=1 1 1 1 itr=1 ire=1 mse=1 tpe=1 lcal=1 sent\n");
       send12_3in1(2,0,0xf);   /* s1=s2=s3=s4=1 */
       send4_3in1(1,1);   /* itr=1 */
       send4_3in1(0,1);   /* ire=1 */
       send4_3in1(3,1);   /* mse=1 */
       send4_3in1(4,1);   /* tpe=1 */
       send4_3in1(5,1);   /* lcal=1 */
       read_status(&sval);   /* read back the controller shift register */
       i = (int)sval;
       s1 = (i>>3) & 1;
       s2 = (i>>4) & 1;
       ...
       printf("s1..4=%d %d %d %d itr=%d ire=%d mse=%d tpe=%d lcal=%d %x\n"
           ,s1,s2,s3,s4,itr,ire,mse,tpe,lcal,i);
       sleep(1);
       printf("\ns1..4=1 0 1 0 itr=1 ire=0 mse=1 tpe=0 lcal=1 sent\n");
       send12_3in1(2,0,0x5);   /* s1=s3=1 s2=s4=0 */
       send4_3in1(1,1);   /* itr=1 */
       send4_3in1(0,0);   /* ire=0 */
       send4_3in1(3,1);   /* mse=1 */
       send4_3in1(4,0);   /* tpe=0 */
       send4_3in1(6,1);   /* lcal=1 */
       read_status(&sval);   /* read back the controller shift register */
       i = (int)sval;
       s1 = (i>>3) & 1;
       s2 = (i>>4) & 1;
       ...
       printf("s1..4=%d %d %d %d itr=%d ire=%d mse=%d tpe=%d lcal=%d %x\n"
           ,s1,s2,s3,s4,itr,ire,mse,tpe,lcal,i);
       sleep(1);
       printf("\ns1..4=0 1 0 0 itr=0 ire=0 mse=0 tpe=0 lcal=0 sent\n");
       send12_3in1(2,0,2);   /* s2=1 s1=s3=s4=0 */
       send4_3in1(0,0);   /* ire=0 */
       send4_3in1(1,0);   /* itr=0 */
       send4_3in1(3,0);   /* mse=0 */
       send4_3in1(4,0);   /* tpe=0 */
       ...
       printf("s1..4=%d %d %d %d itr=%d ire=%d mse=%d tpe=%d lcal=%d %x\n"
           ,s1,s2,s3,s4,itr,ire,mse,tpe,lcal,i);
       sleep(1);
       printf("\ns1..4=0 0 0 0 itr=0 ire=1 mse=0 tpe=0 lcal=0 sent\n");
       send12_3in1(2,0,0);   /* s1=s2=s3=s4=0 */
       send4_3in1(0,1);   /* ire=1 */
       send4_3in1(1,0);   /* itr=0 */
       send4_3in1(3,0);   /* mse=0 */
...
loop11:
   ...
   if(mode == 12)
    {  send4_3in1(4,1);   /* enable tp pulses */
       send4_3in1(5,0);   /* disable ltp pulses */
       printf("input fine dac setting (integer)\n");
       fscanf(stdin,"%d",&ll);
       dval = (unsigned short)ll;      
       send12_3in1(6,1,dval);  /* set fine dac value */
       printf("input coarse dac setting (integer)\n");
       fscanf(stdin,"%d",&ll);
       dval = (unsigned short)ll;      
       send12_3in1(6,0,ll);  /* set coarse dac value */
...
loop14:
       ...
       isw[0]=1;
       send12_3in1(2,0,1);
       send12_3in1(6,0,0);  /* set coarse dac value=0 */
       sleep(2);
       go_get(&ped[0]);
       send12_3in1(6,0,dacset);  /* set coarse dac value */
       sleep(2);
       go_get(&volt[0]);
    ...
    { while(sw(1)==0)
      {for(i=0;i<=0xfff;i++)
        { cadr = (unsigned short)( (zone<<12) | (sector<<6) | i );
          ...
          shift_set(sval);
          shift_read(&dval);
          if(sval != (dval&0xfff) ) printf(" error sval=%x  dval=%x\n",
                  (int)sval,(int)dval);
         }
      }
    }
...
loop19:
       ...
       sw1 = sw(1);
       if(sw1 == 1) 
       printf("card=%d s1-4=%d %d %d %d itr=%d ire=%d mse=%d tpe=%d lcal=%d\n",
              l,s1,s2,s3,s4,itr,ire,mse,tpe,lcal); 
         if( (s1==1) && (s2==0) && (s3==1) && (s4==0) && (ire==0)
            && (itr==1) && (mse==1) && (tpe==0) && (lcal==1) )
   ...
   if(mode == 21)
    {loop21:
       printf("enter coarse dac value (integer)\n");
       fscanf(stdin,"%d",&i);
       printf("coarse dac set to %x\n",i);
       dval = (unsigned short)i;
       send12_3in1(6,0,dval);  /* set coarse dac value */
       printf("enter fine dac value (integer)\n");
       fscanf(stdin,"%d",&i);
   ...
   if(mode == 22)
    { send12_3in1(6,1,255);  /* set fine dac value */
loop22:
       send12_3in1(6,0,255);  /* set coarse dac value */
       usec=5;
       usleep(usec);
       send12_3in1(6,0,127);  /* set coarse dac value */
       usec=5;
       usleep(usec);
       goto loop22;
    }
