...
{  int isw[6],i;
   ...
   fflush(stdout);
    ...
    dacc = (int) dacval;
    set_dac(ttcadr,dacc);
    charge = ( 2.0 * 4.096 * capacitor *(double)dacc ) / 1023.0;
    printf(" charge=%f  dacval=%f  dac=%x(hex)  cap=%f\n",
           charge,dacval,dacc,capacitor);
    ...
    for(tim=0;tim<256;tim++)
      { set_mb_time(ttcadr,board,tim);
        ...
        for(k=0;k<10;k++)
         {inject();
          fermi_read();
	  if(fdata[kk][ipk[kk]] < 500) printf(" timeline error\n");
/*     display ? */
           sw1 = sw(1);
           if(sw1 == 1) 
            { asum = 0.0;
...
evdisp(nn,string,kk)
 ...
 { int max,mmax,idx,idy,ny,linx,ixbas,iybas,ix,iy,iylst,kx,ky,i,k,ii;
   ...
   if(max<1600) mmax=1600;
   if(max<1400) mmax=1400;
