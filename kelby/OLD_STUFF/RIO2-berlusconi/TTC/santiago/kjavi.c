...
kjavi_map(int ttcadr)
 { unsigned long length = 400;
   unsigned long adrmodifier = 0x39;
   unsigned long offset = 0;
   unsigned long size = 0;
   kjavi = (struct kjaviBlock *)(find_controller(0xb00080, length, adrmodifier,
				 offset, size, &param) );
   if(kjavi == (struct kjaviBlock *)(-1)) 
    { printf("Unable to map VME address\n"); }
   usleep(1);
   /*  printf(" csr1=%x  csr2=%x\n",kjavi->csr1,kjavi->csr2);  */
 }

/* ----------------------------------------------------------- */
GetTriggerSelect()
 { return kjavi->csr1 & 0x7; }
...
SetTriggerSelect(unsigned int tsel)
 { kjavi->csr1 = (kjavi->csr1 & 0xfff8) | (tsel & 0x7);
   /* printf("csr1=%x\n",kjavi->csr1);  */
 }
...
/* ----------------------------------------------------------- */
GetRandTrigRate()
 { return (kjavi->csr1 >> 12) & 0x7; }

/* ----------------------------------------------------------- */
SetRandTrigRate(unsigned short rate)
 { kjavi->csr1 = (kjavi->csr1 & 0x8fff) | ((rate & 0x7) << 12); }

/* ----------------------------------------------------------- */
GetInhibitDelay(int channel)
 { return kjavi->b_go[channel].inhibit_delay; }

/* ----------------------------------------------------------- */
SetInhibitDelay(int channel, unsigned short delay)
 { kjavi->b_go[channel].inhibit_delay = delay & 0xfff; }

/* ----------------------------------------------------------- */
GetInhibitDuration(int channel)
 { return kjavi->b_go[channel].inhibit_duration; }

/* ----------------------------------------------------------- */
SetInhibitDuration(int channel, unsigned short duration)
 {  kjavi->b_go[channel].inhibit_duration = duration & 0xff; }

/* ----------------------------------------------------------- */
BroadcastShortCommand(unsigned int command)
 { kjavi->short_cycle = (unsigned short)command; }
...
/*  intvl.tv_sec=0;
   ...
   kjavi->long_cycle = lwd;
 }

/* ----------------------------------------------------------- */
GetBMode(int channel)
 { return kjavi->b_go[channel].mode; }

/* ----------------------------------------------------------- */
SetBMode(int channel, unsigned char mode)
 { kjavi->b_go[channel].mode = mode & 0xf; }

/* ----------------------------------------------------------- */
SetRetransmit(int channel, int retransmit)
 { kjavi->csr2 = (kjavi->csr2 & ~(1u << 8 + channel))
    | (!retransmit << 8 + channel);
 }

/* ----------------------------------------------------------- */
ResetFIFO(int channel)
 { kjavi->csr2 = (kjavi->csr2) | (1u << 12 + channel); }

/* ----------------------------------------------------------- */
BPutShort(int channel, int command) 
  { unsigned long kl;
    kl =  (command<<23); 
    kjavi->bgo_param[channel] = kl;  
  }
...
BPutLong(int channel, int address, int external, int subadr, int data) 
 { unsigned long lwd;
   lwd = 0x80000000 | (address<<17) | (external<<16) | (subadr<<8) | data;
   kjavi->bgo_param[channel] = lwd;
 }

/* ----------------------------------------------------------- */
BGoSignal(int channel)
 { kjavi->b_go[channel].sw_go = 0; }
