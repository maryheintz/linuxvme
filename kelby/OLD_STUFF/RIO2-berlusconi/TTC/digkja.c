...
/*    if(drawer==5) ttcadr = 0x2004;  */
    if(drawer==5) ttcadr = 0x3807;
    printf("ttcadr = %x\n",ttcadr);  
    id=1;
    ttcvi_map(0);
    SendLongCommand(0,0,0x3,0xa3); /* reset ttcrx chip */
    ttcvi_map(ttcadr);
    k = GetTriggerSelect();
...
printf(" calling digitizer setup\n");
   ...
   /* for now set up the digitizer ttcrx chips using adrttc=0 */
	digitizer_setup(0);  /* for some reason - must be called twice */
  	digitizer_setup(0);     
...
digitizer_setup(int adrttc)
      { int dped;
        
	SendLongCommand(adrttc,0,0x3,0xa3); /* reset ttcrx chip */
	usleep(200);
...
/* force high gain */
	  ...
	  { SendLongCommand(adrttc,1,0xb,0xff); /* high limit LSB=0xff */
	    ...
	    SendLongCommand(adrttc,1,0xc,0xff); /* high limit MSB=0xff */
            usleep(200);

            SendLongCommand(adrttc,1,0xd,0xff);  /* low limit LSB=0xff */
	    usleep(200);
	
	    SendLongCommand(adrttc,1,0xe,0xff);  /* low limit MSB=0xff */
	    usleep(200);
...
/* do low gain */
	...
	dped=200;
	if(sw(1) == 1)
	  {printf("default ped=%d\n",dped);
	   printf("input new ped value\n");
	   fscanf(stdin,"%d",&dped);
	  }
	SendLongCommand(adrttc,1,0x9,dped);  /* set dac */	
	usleep(200);	
	return;
	}
...
slink_read()
	...
	{ if (type_of_word != SLINK_CONTROL_WORD) 
	 	{               kk = count/16;
				...
				sd.ldata =  word;
				if(sw(8) == 0) 
				{ printf("count=%d kk=%d  ii=%d data word= %x  %x  %x   word=%x\n",
				  count,kk,ii,sd.bits.data1,sd.bits.data2,sd.bits.data3,word); 
				}
				if(kk<8)
	 ...
	 if(sw(8) == 0) printf("no data  code=%x  count=%d\n",code,count);
	return;
error:
	 if(sw(8) == 0) printf("error  kk=%d  ii=%d  count=%d\n",kk,ii,count);
	return;
       }
