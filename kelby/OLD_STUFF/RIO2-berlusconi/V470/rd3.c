...
int dummy[2000];
double adc[30000],fdac[30000],dadc[30000];
int ndac;
FILE *fp,*fpascii;
main(argc,argv,envs)
int argc;
...
char *envs[];
  { int i,k,nwds;
    ...
    dotset(5,96.0,32.0,5.0,0.-5,0,0);
...
/*    printf("nwds=%d\n",nwds);  */
/*    printf(" dat.dac=%d\n",dat.dac);  */
    if(dat.dac>13900) goto xit;
    dac = dat.dac+dacoffset;
    xadc = dac/27.0;
    avg = dat.av;
/*    if(avg>1022.0) goto loop;  */
    davg = dat.dav;
    dotacc(0,dac,avg);
    dotacc(1,xadc,avg);
   ...
   for(k=0;k<ndac;k++)
     { y = a + b*fdac[k];
      ...
      /* printf(" k=%d  x=%f  y=%f  adc=%f  dy=%f\n",k,x,y,adc[k],dy); */
       ...
       dotacc(8,xadc,dy/y);
       dotacc(5,xadc,dy);
    fprintf(fpascii,"%f %f %f %f\n",fdac[k],adc[k],dadc[k],y);
     }
/*    dotwrt(0); 
    dotwrt(1); 
    dotwrt(2); 
