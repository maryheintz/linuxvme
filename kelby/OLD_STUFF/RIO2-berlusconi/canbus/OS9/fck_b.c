...
/* set the zone and sector number of the drawer being addressed */
   set_zone(zone);
   set_sector(sector);
/* set the time delays on each drawer section for equal time pulsing */
/*    warning ######## set_mb_time resets the tube number ####### */
   set_mb_time(1,127);
   set_mb_time(2,76);
   set_mb_time(3,68);
   set_mb_time(4,25);
/*   find the first card present on the drawer */
   itst = 0x1eaa;
   for(k=0;k<49;k++)
     { can_tube(k);
         i=0xaaa;
