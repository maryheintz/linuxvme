...
#define IBLCL			/* IB "local" symbols */

#endif				/* ------------------------------------ */

#define GPIBin(x)    (ib->x)	/* Interface board access macros... */
...
uint8     ibvec    = IBVEC;	/* interrupt vector number for GPIB       */
uint8     ibirq    = IBIRQ;	/* interrupt request line for GPIB (1-7)  */
uint8     ibbrg    = IBBRG;	/* bus request/grant line for GPIB (1-3)  */
...
IBLCL struct ibregs *ib = ((struct ibregs *) 0xdffe2000);
...
rddone:
	...
	ibcnt = cnt - dma_stop(buf, cnt, D_DTM, ccfunc);
	...
	if ((ibsta & END) || (!no_HLDA && (ibcnt == cnt)))
		pgmstat |= PS_HELD;
...
unsigned int cnt;
{
	...
	GPIBout(auxmr, AUX_GTS);	/* if CAC, go to standby */
	requested_cnt = cnt;
/*kja	while ((cnt > 0) && !(ibsta & (ERR | TIMO))) {  */
	while ((cnt > 0) && !(ibsta & ERR )) {
		ibcnt = 0;
...
wrtdone:
	...
	ch1csr = GPIBin(ch1.csr);
	DBGprint(DBG_DATA, ("ch0csr=0x%x ch1csr=0x%x  ", ch0csr, ch1csr));
	ibcnt = cnt - dma_stop(buf, cnt, D_MTD, ccfunc);
...
unsigned int cnt;
{
	...
	DBGprint(DBG_BRANCH, ("begin PIO loop  "));
	while (ibcnt < cnt) {		/* NOTE: first CO assumed */
		GPIBout(cdor, buf[ibcnt++]);
...
IBLCL ibstat()			/* update the GPIB status information */
{
	...
	DBGin(IBSTAT);
	ibsta |= (GPIBin(ch0.csr) & D_PCLH) ? 0 : SRQI;
	brdstat = GPIBin(adsr);
...
int v;
{
	...
	DBGin(IBONL);
/*KJA	ib = (struct ibregs *) ibreg_addr(ibbase);  */
/*
...
/*kjakja	for (i = 0; i < 10000; i++);  */
        usleep((time_t)20000);                /* delay at least 10 msecs. */
	ibcfg2 = 0;
	GPIBout(cfg2, ibcfg2);                      /* clear LMR */
	ibcfg1 = ((ibirq << 5) | (ibbrg << 3) | D_DBM);
	...
	GPIBout(ch0.niv, ibvec);
	GPIBout(ch0.eiv, ibvec);
	GPIBout(ch1.niv, ibvec);
	GPIBout(ch1.eiv, ibvec);
	GPIBout(ch0.cpr, 3);
	GPIBout(ch1.cpr, 3);
...
/*kjakja	for(i = 0; i < ifcDelay; i++);  */
        usleep((time_t)200);              /* busy wait >= ~100us */
