...
char *envs[];
  { FILE *fp;
    ...
    int nevnt,nevmax,nstrtev,l,nn,i,k,wait,kl,lk,start;
    int ipeak,timc,timf;
    int card;
   ...
   for(kl=start;kl<49;kl=kl+2) 
       { if(cardchk(kl) == 1) 
         { printf(" connect card %d",kl);
	   lk=getchar();
            card = kl;
...
/* --------------------- determine higain timing -------------------*/
    ...
    if(sw(4) == 1) printf("higain charge=%f dac=%d capacitor=%f\n",
...
/*  now get mean  */
   ...
   if(sw(4) == 1) printf("high gain timer=%d channel=%6.1f peak=%5.1f\n",
    timf,meanhp,meanhc);
   if(meanhc>pmaxh)
    { pmaxh=meanhc;
      cmaxh=meanhp;
...
/* --------------------- determine logain timing -------------------*/
    ...
    dac = (int)fdac;
    charge = 2.0 * 4.096 * capacitor * (double)dac / 1023.0;
    if(sw(4) == 1) printf("logain charge=%f dac=%d capacitor=%f\n",
         charge,dac,capacitor);
    set_dac(ttcadr,dac);
    inject();  
   i=0;
...
/*  now get mean  */
   ...
   if(sw(4) == 1) printf(" low gain timer=%d channel=%6.1f peak=%5.1f\n",
    timf,meanlp,meanlc);
   if(meanlc>pmaxl)
    { pmaxl=meanlc;
      cmaxl=meanlp;
...
/* ---------------- high gain calibration ----------------------*/
   ...
   khi=kpeak+2;
   timf=(int)(tmaxh+0.5);
   if(sw(4) == 1) printf("starting higain  kpeak=%d timf=%d\n",kpeak,timf);
   kpeakh = kpeak;
   timfh = timf;
   SetTimeDes2(ttcadr,timf);  
   icnt=0;
...
/*     display ? */
	...
	if(ped<10.0) printf("error ped=%f axsum=%f asum=%f\n",ped,axsum,asum);
        ...
        if(sig2>0.0) dped=sqrt(sig2);
         hstacc(3,dped,1.0);
        kpk=ipk[1];
        if(fdata[kpk][1]>3950) goto fithi;
        val = (double)fdata[1][kpeak] - ped;
	if(val<0.0) continue;
        if(ipk[1]>=klo && ipk[1]<=khi && fdata[1][kpeak]<3950)
         { sev = sev + 1.0;
           sxev = sxev + val;
           sxxev = sxxev + val*val;
	 }
        ...
        if(sev>1.0) 
          {val = sxev / sev;
           if(sw(4) == 1) printf(" high gain dac=%d charge=%f fmean=%f\n",
           dac,charge,val);
           dotacc(4,charge,val);
           sig=0.0;
...
fithi:
   ...
   a = (sx2*sy - sx*sxy) / del;
   b = (s*sxy - sx*sy) / del;
   if(sw(4) == 1) printf(" higain fit results a=%f b=%f y=a+b*charge\n",a,b);
   ah = a;
   bh = b;
   for(k=0;k<icnt;k++)
    {x = crg[k];
...
/*     display ? */
        ...
        hstacc(2,dped,1.0);
        kpk=ipk[2];
        if(fdata[2][kpk]>3950) goto lofit;
        val = (double)fdata[2][kpeak] - ped;
	if(val<0.0) continue;
        if(ipk[2]>=klo && ipk[2]<=khi && fdata[2][kpeak]<3950)
         { sev = sev + 1.0;
           sxev = sxev + val;
           sxxev = sxxev + val * val;
         }
        ...
        if(sev>0.0) 
          {val = sxev / sev;
           if(sw(4) == 1) printf("  low gain dac=%d charge=%f fmean=%f\n",
           dac,charge,val);
           dotacc(5,charge,val);
           sig=0.0;
           sig2=sxxev/sev - val*val;
...
lofit:
   ...
   al = a;
   bl = b;
   if(sw(4) == 1) printf(" logain fit results a=%f b=%f y=a+b*charge\n",a,b);
   for(k=0;k<icnt;k++)
    {x = crg[k];
     y = a + b*x;
     dely = y - fvl[k];
...
/*     fprintf(fp,"%4d %5d %4d %9.2f %6.2f %4.1f %5.2f %5d %4d %8.2f %4.2f %4.1f %5.2f\n",
        ...
        ratio = bh/bl;
    if(sw(4) == 1) printf("card=%d bh=%f  bl=%f  ratio=%f\n",card,bh,bl,ratio);
/*    sidev(0);
     dotmwr(2,0,1);
     dotmwr(2,2,3);
     dotwrt(4);
...
/*----------------------------- check trigger output ------------- */
    ...
    set_dac(ttcadr,dac);
    for(k=0;k<3;k++)
     { if(k == 0) trig_enable(ttcadr);
       if(k == 1) trig_disable(ttcadr);
       if(k == 2) trig_enable(ttcadr);
       usleep(500);
       inject();  
        fermi_read();
        kpk=ipk[0];
        ...
        for(i=10;i<20;i++)
         ...
         if(sig2>0.0) dped=sqrt(sig2);
         sprintf(string,
           "trig output ipk=%d  pk=%d  ped=%f +/- %f",
              ipk[0],pk[0],ped,dped);
         evdisp(50,string,0);
      }
