...
char *envs[];
  { int tim,i,l,k,kkk,lll,pipe,drawer,dig,kk,tube;
    ...
    fprintf(fp," charge=%f  dacval=%f  dac=%x(hex)  cap=%f\n",
...
/*   SetTimeDes1(0x1005,kkk); */
       ...
       slink_read();
               if(sw(8) == 1)
	         {usleep(10000);
		  if(sw(7) == 0) fprintf(fp,"next event\n");
		  goto loop;}
...
int k;
  {int i,status;
   ...
   if((s1==1) && (s2==0)  && (s3==1) && (s4==0) && (itr==1) &&
      ...
      (trig==0) ) ok++;
   if(sw(2)==1) fprintf(fp,"k=%d s1..4=%d %d %d %d itr=%d ire=%d mse=%d tplo=%d tphi=%d trig=%d %x\n"
...
slink_setup()
	...
	{
	fprintf(fp," slink open error, error code = %d\n",code);
	SLINK_PrintErrorMessage(code);
	exit(0);
	}
	fprintf(fp,"slink opened\n");
	return; 
	}
...
slink_read()
	...
	int parity,samples,error,sestr,destr,r_parity,v_parity,
	   mode,gain,bunch,cc;
	int type_of_word,count,kk,ii,stop;
	 ...
	 if (type_of_word != SLINK_CONTROL_WORD) 
	 	{
				...
				if(ii == 0) 
				   { parity = (word>>30) & 1;
		                     ...
		                     gain = (word>>12) & 0x7;
		                     bunch = word & 0xfff;
		                     if(sw(7) == 0) fprintf(fp,"header word %x   count %d\n",word,count);
		                     if(sw(7) == 0) fprintf(fp," parity=%d  samples=%d  error=%d\n",
		                        parity,samples,error);
		                     if(sw(7) == 0) fprintf(fp," SEstr=%d   r_parity=%d   v_parity=%d\n",
		                        sestr,r_parity,v_parity);
		                     if(sw(7) == 0) fprintf(fp," mode=%d   gain=%d   bunch=%d\n",
		                       mode,gain,bunch);
				   }
				/* else if(ii==15) 
				   {if(sw(7) == 0) fprintf(fp," crc %x count %d\n",word,count); } */
				else
				  { if(sw(7) == 0) fprintf(fp," data word= %4i %4i %4x %4x %4x",
	                              sd.bits.topbit,sd.bits.parity,
				      sd.bits.data1,sd.bits.data2,
				      sd.bits.data3);
				    if(sw(7) == 0) fprintf(fp,"   %d  %d  count %d  %x\n",kk,ii,count,word);
				  } 
				if(kk<8)
	       ...
	       { cc++;
	         if(sw(7) == 0) fprintf(fp,"control word= %x\n", word);
		 if(cc == 1 && word != 0x11111110) 
		   { nerror++;
		     fprintf(fp,"control word= %x  cc %d  error %d  event %d\n",
