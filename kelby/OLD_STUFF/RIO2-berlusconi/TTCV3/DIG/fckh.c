...
/*  now get mean  */
   ...
   printf(" high gain timer=%d sum=%f channel=%f peak=%f\n",
...
/*     display ? */
	...
	dped = 0.0;
	  ...
	  { printf("overflow ped=%f  fdata=%d  charge=%f\n",
	...
	if(val<0.0) break;
        ...
        if(sev>0.0) 
          {val = sxev / sev;
           ...
           printf("  high gain dac=%d charge=%f fmean=%f  sig=%f\n",
           dac,charge,val,sig);
          }
...
lofit:
   printf(" s=%g  sx=%g  sy=%g  sxy=%g  sx2=%g\n",s,sx,sy,sxy,sx2);
   del = s*sx2 - sx*sx;
   a = (sx2*sy - sx*sxy) / del;
   b = (s*sxy - sx*sy) / del;
   al = a;
