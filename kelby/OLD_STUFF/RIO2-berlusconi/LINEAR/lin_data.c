...
chk_digital()
  {int i,status;
   ...
   if(sw(3)==1) printf("k=%d s1..4=%d %d %d %d itr=%d ire=%d mse=%d tplo=%d tphi=%d trig=%d %x\n"
           ,card,s1,s2,s3,s4,itr,ire,mse,tplo,tphi,trig,i);
...
chk_fast()
  { double asum,axsum,axxsum,ped,dped,sig2,aval,yc;
    ...
    tmaxl=0.0;
        ...
        dotset(1,250.0,0.0,15.0,5.0,0,0);
        sprintf(xstring,"fine time setting");
         ...
         if(sw(1) == 1) 
          { asum = 0.0;
            ...
            sprintf(string,
              ...
              ipk[card],pk[card],ped,dped);
            evdisp(16,string,card);}
/* now gets sums for mean calculation */
       sum=sum+1.0;
       kipk = ipk[card];
       yc=(double)fdata[card][kipk];
       xhsumc=xhsumc+yc;
...
/* --------------------- determine logain timing -------------------*/
    capacitor = 100.0;
    small_cap_disable(ttcadr);   /* disable small C */
    large_cap_enable(ttcadr);   /* enable large C */
    charge=500.0;
    fdac = (1023.0*charge) / (2.0*4.096*capacitor);
    dac = (int)fdac;
    charge = 2.0*4.096 * capacitor * (double)dac / 1023.0;
...
/*     display ? */
        ...
        if(sig2>0.0) dped=sqrt(sig2);
        hstacc(3,dped,1.0);
        if(fdata[card][kipk]>1020) goto fithi;
        val = (double)fdata[card][kpeak] - ped;
        el = (double)fdata[card][kpeak-1] - ped;
        er = (double)fdata[card][kpeak+1] - ped;
        ep = val;
        estimator = (el-er)/ep;
        if(kipk>=klo && kipk<=khi && fdata[card][kpeak]<1020)
         { sev = sev + 1.0;
           sxev = sxev + val;
           sxxev = sxxev + val*val;
        ...
        if(sev>0.0) 
          {val = sxev / sev;
           estimator = sxest / sev;
           printf(" high gain dac=%d %x charge=%f fmean=%f\n",
           dac,dac,charge,val);
           dotacc(4,charge,val);
           sig=0.0;
...
/*     display ? */
        ...
        if(sig2>0.0) dped=sqrt(sig2);
        hstacc(2,dped,1.0);
        if(fdata[card][kipk]>1020) goto lofit;
        val = (double)fdata[card][kpeak] - ped;
        el = (double)fdata[card][kpeak-1] - ped;
        er = (double)fdata[card][kpeak+1] - ped;
        ep = val;
...
lofit:
   ...
   for(k=0;k<icnt;k++)
    {x = crg[k];
     ...
     if(sw(3) == 1) printf("k=%d x=%f  y=%f  fvl[k]=%f  dely=%f\n",k,x,y,fvl[k],dely);
     dotacc(7,x,dely);
    fprintf(fp,"%8.2f\t%8.2f\t%8.3f\n",crg[k],fvl[k],dely);
    }
    dotwrt(7);
    fprintf(fp," logain fit a=%f  b=%f\n",a,b);
    dymaxl = factor * (sx/s);
...
evdisp(nn,string,kk)  /* nn=bins to display  kk=card number */
 ...
 { int max,mmax,idx,idy,ny,linx,ixbas,iybas,ix,iy,iylst,kx,ky,i,k,ii,iii;
   ...
   erasm();
   max = 0;
   for(iii=1; iii<=nn; iii++) { printf(" %d  %d\n",iii,fdata[kk][iii]);
