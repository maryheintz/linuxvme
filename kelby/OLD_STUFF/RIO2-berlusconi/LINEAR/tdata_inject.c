...
char *envs[];
  { int tim,i,l,k,kkk,lll,drawer,dig,kk,tube;
    int dac,alarm,is,kja1=1,kja2=3;
    int good,bad,total,board;
    double charge,dacval,capacitor;
    char xstring[80],ystring[80],string[80],err[80];
    double asum,axsum,axxsum,ped,dped,sig2,aval,yc,bxsum;
    char serial[7] = {"S10135\0"};
    tube=0;
    kk = 0;
    printf("input tube and charge\n ");
    fscanf(stdin,"%d %lf",&tube,&charge);
    nsamp = 15;
/*    printf("input nsamp\n ");
   ...
   hstset(0,500.0,0.0,50,1);
   hstlbl(0,"kja1 charge distribution");
   hstset(1,1000.0,0.0,200,1);
   hstlbl(1,"kja1 charge distribution");
   hstset(2,500.0,0.0,50,1);
   hstlbl(2,"kja2 charge distribution");
   hstset(3,1000.0,0.0,200,1);
   hstlbl(3,"kja2 charge distribution");
    ...
    for(k=1;k<49;k++)
	  ...
	  if(k==tube) 
	     { printf(" enabling small cap for tube 1\n"); 
...
/*    printf(" calling slink_setup again\n");
	slink_setup(); */
...
/*  	  if(sw(7) == 0) twodisp(16,string,5,6);  */
          sprintf(string,"above tube %d --- below tube %d  gain=%d\n",
	     kja2,kja1,evgain);
  	  if(sw(7) == 0) twodisp(16,string,kja1,kja2);
              if(sw(1) == 1)
   		{ if(evgain == 0) sprintf(string,"LOW GAIN TUBES 1-24");
  		  if(evgain == 7) sprintf(string,"HIGH GAIN TUBES 1-24");
  		  bigdisp(nsamp+1,string,1);
  		  if(evgain == 0) sprintf(string,"LOW GAIN TUBES 25-48");
  		  if(evgain == 7) sprintf(string,"HIGH GAIN TUBES 25-48");
		  ...
		  bxsum = 0.0;
                  ...
                  for(i=1;i<8;i++)
                   { aval = (double)fdata[kja1][i];
                     asum = asum+1.0;
                     axsum = axsum + aval;
		     bxsum = bxsum + (double)fdata[kja1][i+7];
                     axxsum = axxsum + aval*aval;
                   }
                 ped = axsum / asum;
		 hstacc(0,bxsum-axsum,1.0);
		 hstacc(1,bxsum-axsum,1.0);
		  ...
		  bxsum = 0.0;
                  axxsum = 0.0;
                  for(i=1;i<8;i++)
                   { aval = (double)fdata[kja2][i];
                     asum = asum+1.0;
                     axsum = axsum + aval;
		     bxsum = bxsum + (double)fdata[kja2][i+7];
                     axxsum = axxsum + aval*aval;
                   }
                 ped = axsum / asum;
		 hstacc(2,bxsum-axsum,1.0);
		 hstacc(3,bxsum-axsum,1.0);
          if(sw(6) == 1) 
	    { hstwrt(0);
	      hstwrt(1);
	      hstwrt(2);
...
twodisp(nn,string,kk1,kk2)  /* nn=bins to display  kk1,kk2=card numbers */
 int nn,kk1,kk2;
 char *string;
