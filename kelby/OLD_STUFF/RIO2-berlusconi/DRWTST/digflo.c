...
int code;
static int nsamp;
int event=0,nerror=0,ndcrc=0,nbnch=0,ngcrc=0,nctrl=0,nsdmu=0;
...
/*    printf("input nsamp\n ");
   ...
   printf(" good=%d    bad=%d     %d\n",good,bad,total);
    ...
    BPutShort(0,1);   /* reset bunch counter on inh0  */
     ...
     nctrl=0;
     nsdmu=0;
/* toss 2 events to clear any junk in buffer */
     clear_buffer();
     clear_buffer();
 loop:
       event++;
       if(sw(3) == 0 && (event%50) == 0) 
             printf("event=%d  nerror=%d  ndcrc=%d  nbnch=%d  nctrl=%d  ngcrc=%d  nsdmu=%d\n",
               event,nerror,ndcrc,nbnch,nctrl,ngcrc,nsdmu);
       if(sw(3) != 0 && (event%1000) == 0) 
             printf("event=%d  nerror=%d  ndcrc=%d  nbnch=%d  nctrl=%d  ngcrc=%d  nsdmu=%d\n",
               event,nerror,ndcrc,nbnch,nctrl,ngcrc,nsdmu);
       slink_read();
              if(sw(8) == 1)
	...
	{if (type_of_word != SLINK_CONTROL_WORD) 
	 	{ kk = count/(nsamp+3);
		  ...
		  if(kklast == 15 && ii == 0) 
		   ...
		   { parity = (word>>30) & 1;
			...
			{printf("hcnt error count=%d\n",count);
			 ...
			 return;
		        ...
		        if(sw(7) == 1) 
			 printf("%d header word %x  bunch %d   %d\n",hcnt,word,bunch,count);
		   }
		 else if(ii>nsamp+1) 
		   { crc1 = (word>>16);
		     crc2 = word & 0xffff;
		   ...
		   { crc(1,word);
		     crcaa = crca & 0x7fff;
                     if(sw(7) == 1) printf(" data word= %4i %4i %4x %4x %4x    %8x  %4x %4x\n",
		 ...
		 if(kk<8)
	       ...
	       if(sw(7) == 1)
	         { if(cc == 1 && word == 0x51115110) printf("\n\n");
	           printf("A control word= %x\n", word);
		 }
		 if(cc == 1 && word != 0x51115110) 
		   { ctrlok++;
		     printf("B control word= %x  cc %d  error %d  event %d\n",
		      word,cc,nerror,event);
		     if(sw(5) == 1) fscanf(stdin,"%d",&stop); 
		   } 
		 if(cc == 2 && word != 0xfffffff0) 
		   { ctrlok++;
		     printf("C control word= %x  cc %d  error %d  event %d\n",
		      word,cc,nerror,event);
		     /* sleep(10);  */
		     if(sw(5) == 1) fscanf(stdin,"%d",&stop);
		     return; 
