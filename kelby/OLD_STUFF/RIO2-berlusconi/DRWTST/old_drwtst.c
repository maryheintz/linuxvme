...
int hsamp[49][240],hmax[49],hbmax[49];
...
int ddac1[9]={100,100,100,100,100,100,100,100,100};
int ddac2[9] ={100,100,100,100,100,100,100,100,100};
unsigned int deskew1[9] = {0,0,0,0,0,10,0,0,0};
unsigned int deskew2[9] = {100,150,165,115,150,80,50,55,40};
unsigned int pipeline[9] = {100,23,23,24,23,24,24,24,24};
...
double fl[12]={0.0,122.0,97.0,87.0,-5.0,0.0,4.6,1.31,-1,8,0.0,1.61,72.0};
...
/* check that ARBLOGIC is working */
    ...
    *reg = 0x1234;
    ...
    hstset(2,150.0,50.0,100,1);
    hstlbl(2,"higain phase");
    hstset(3,150.0,50.0,100,1);
    hstlbl(3,"logain phase");
    hstset(4,100.0,50.0,100,1);
...
/* ================================================================ */
    ...
    small_cap_enable(ttcadr);
	 ...
	 kipk = 4;  
	 dotzro();
	 dotset(0,10.0,0.0,1000.0,0.0,0,0);
	 dotset(1,10.0,0.0,20.0,-20.0,0,0);
	 dotset(2,10.0,0.0,10.0,0.0,0,0);
...
/* ==================== high gain dac loop ============== */
   ...
   set_dac(ttcadr,0);
   usleep(2000);
   for (dac=0; dac<240; dac=dac+dstep)
     {set_dac(ttcadr,dac);
      kwait(10);
      volt = 2.0*4.096 * (double)dac/1023.0;
      charge = volt * capacitor;
      if(charge>10.0) continue;
      sev = 0.0;
      sxev = 0.0;
      sxxev = 0.0;
      sxest = 0.0;
...
/*     go read the  module ------------------------------- */
        slink_read();
/*	printf(" fdhi= %d %d %d %d %d %d %d\n",
	   fdhi[card][0],fdhi[card][1],fdhi[card][2],fdhi[card][3],
	   fdhi[card][4],fdhi[card][5],fdhi[card][6]); */
/*     display  */
         if(sw(7) == 1) 
          { asum = 0.0;
            axsum = 0.0;
            ...
            sprintf(string,"hi gain ipk=%d  pk=%d  ped=%f +/- %f",
        ...
        sig2=axxsum/asum - ped*ped;
        if(sig2>0.0) dped=sqrt(sig2);
        if(fdhi[card][kipk]>1000) goto fithi;  
        val = (double)fdhi[card][kipk] - ped;
        sev = sev + 1.0;
        sxev = sxev + val;
        sxxev = sxxev + val*val;
      } /* end of nevnt loop */
fithi:  val=0.0;
	   ...
	   dotacc(2,charge,sig);
	     ...
	     pkhis[card][n] = sig;
	     npkhi[card] = n;
	     if(sw(5)==1) printf(" card=%d  pk=%f  sig=%f  ped=%f crg=%f\n",card,pk,sig,ped,charge);
             n++;
          }
           icnt++; 
   }  /* end of dac loop */
...
/* ========= high gain linearity == fit the data to a line ============ */
	 ...
	 del = s*sx2 - sx*sx;
	 ah[card] = (sx2*sy - sx*sxy) / del;
	 bh[card] = (s*sxy - sx*sy) / del;
	 if(bh[card]<fl[3]-dfl[3] || bh[card]>fl[3]+dfl[3])
	  { FAIL[icard] = FAIL[icard] | 0x8;
	    printf(" card %d FAILED higain slope %f  expected=%f\n",card,bh[card],fl[3]);
	  }
	 if(ah[card]<fl[4]-dfl[4] || ah[card]>fl[4]+dfl[4]) 
	  { FAIL[icard] = FAIL[icard] | 0x10;
	    printf(" card %d FAILED higain intercept %f  expected=%f\n",card,ah[card],fl[4]);
	  }
	 hstacc(4,bh[card],1.0);
	 hstacc(5,ah[card],1.0); 
/*============  plot the difference between the line and the data =========== */
	 for(k=0;k<n;k++)
	   { x = pkhic[card][k];
	     y = ah[card] + bh[card] * x;
	     dely = y - pkhi[card][k];
             if(sw(5)==1) printf(" card %d fit  x=%f  y=%f  dely=%f  pkhi=%f\n",card,x,y,dely,pkhi[card][k]);
	     dotacc(1,x,dely);
	     if(abs(dely)>fl[5]+dfl[5]) 
	      { FAIL[icard]= FAIL[icard] | 0x20;
	        printf(" card %d FAILED higain linearity %f  x=%f  y=%f\n",card,dely,x,y);
	      }
	     if(abs(pkhis[card][k])>fl[6]+dfl[6]) 
	      { FAIL[icard] = FAIL[icard] | 0x40;
	        printf(" card %d FAILED higain sigma %f\n",card,pkhis[card][k]);
	      }
	     hstacc(6,dely,1.0);
	     hstacc(7,pkhis[card][k],1.0);
	   }
	 ...
	 if(FAIL[icard] == 0) printf("high gain linearity - card %d OK\n",card);
	 if((FAIL[icard]!=0) && (sw(6)==1)) 
	   { printf(" card %d failed --- <cr> for plot\n",card);
	     dotmwr(3,0,2);
	   }
...
/* ================================================================ */
    ...
    small_cap_disable(ttcadr);
	 ...
	 kwait(100);
         clear_buffer();
         card = icard+1;
         SetTimeDes2(ttcadr,lmax[icard]);
	 kipk=4; 
	 dotzro();
	 dotset(0,800.0,0.0,1000.0,0.0,0,0);
	 dotset(1,800.0,0.0,20.0,-20.0,0,0);
	 dotset(2,800.0,0.0,10.0,0.0,0,0);
	 n = 0;
	 nevmax=10;
	 step = 50;
/* ==================== low gain dac loop ============== */
...
/* =========== low gain linearity == fit the data to a line ============= */
	 ...
	 del = s*sx2 - sx*sx;
	 al[card] = (sx2*sy - sx*sxy) / del;
	 bl[card] = (s*sxy - sx*sy) / del;
	 /* plot the difference between the line and the data */
	 /* printf(" logain fit  card=%d  slope=%f  intercept=%f\n",i+1,bl[card],al[card]); */
	 if(bl[card]<fl[7]-dfl[7] || bl[card]>fl[7]+dfl[7]) 
	  { FAIL[icard] = FAIL[icard] | 0x80;
	    printf(" card %d FAILED logain slope %f  expected=%f\n",card,bl[card],fl[7]);
	  }
	 if(al[card]<fl[8]-dfl[8] || al[card]>fl[8]+dfl[8]) 
	  { FAIL[icard] = FAIL[icard] | 0x100;
	    printf(" card %d FAILED higain intercept %f  expected=%f\n",card,al[card],fl[8]);
	  }
	 hstacc(8,bl[card],1.0);
	 hstacc(9,al[card],1.0);
	 for(k=0;k<n;k++)
	   { x = pkloc[card][k];
	     y = al[card] + bl[card] * x;
	     dely = y - pklo[card][k];
	     dotacc(1,x,dely);
	     /* printf(" k=%d x=%f  yfit=%f  ydata=%f  dely=%f\n",k,x,y,pklo[card][k],dely); */
	     if(abs(dely)>fl[9]+dfl[9]) 
	      { FAIL[icard] = FAIL[icard] | 0x200;
	        printf(" card %d FAILED logain linearity %f  x=%f  y=%f\n",card,dely,x,y);
	      }
	     if(abs(pklos[card][k])>fl[10]+dfl[10]) 
	      { FAIL[icard] = FAIL[icard] | 0x400;
	        printf(" card %d FAILED logain sigma %f  x=%f  y=%f\n",card,pklos[card][k],x,y);
	      }
	     hstacc(10,dely,1.0);
	     hstacc(11,pklos[card][k],1.0);
	   }
         sprintf(xstring,"LOGAIN Linearity Plot card %d  Charge vs Flat Filter Counts",card);
         dotmlbl(xstring); 
	 if(sw(3) == 1) dotmwr(3,0,2);
	 ratio = bh[card]/bl[card];
	 /* printf(" card=%d  ratio=%f\n",card,ratio); */
	 if(ratio<fl[11]-dfl[11] || ratio>fl[11]+dfl[11])
	  { FAIL[icard] = FAIL[icard] | 0x800;
	    printf(" card %d FAILED gain ratio %f  expected=%f\n",card,ratio,fl[11]);
	  }
	 hstacc(12,ratio,1.0);
	 if(FAIL[icard] == 0) printf("low gain linearity - card %d OK\n",card);
	 if((FAIL[icard]!=0) && (sw(6)==1)) 
	   { printf(" card %d failed --- <cr> for plot\n",card);
	     dotmwr(3,0,2);
	   }
