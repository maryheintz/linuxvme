...
int k;
  {int i,status;
   ...
   printf("cardchk 1 readback 3in1\n");
   can_bkld_high(id);
   can_bkld_low(id);
   ...
   printf("cardchk 2 readback 3in1\n");
   can_bkld_high(id);
   can_bkld_low(id);
   status=can_3in1set(id,0xd400);   /* fetch from 3in1 */
   status=can_3in1get(id,&shrt);    /* and load can output reg */
