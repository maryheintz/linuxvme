...
char *envs[];
  { int tim,i,l,k,kkk,lll,pipe,drawer,dig,kk,tube;
    ...
    fpdiag = fopen("/home/user/kelby/DRWTST/digzro.diag","w");
		  ...
		  if(kklast == 15 && ii == 0) 
		   ...
		   { crc1 = (word>>16);
		     ...
		     if(crcaa != crc1 || crcb != crc2) dbad[hcnt-1]++;
		       ...
		       if(crc1==0 || crc2==0)
		         { printf("      tiledmu crc %4x %4x\n",crc1,crc2);
			   printf("   calculated crc %4x %4x\n",crcaa,crcb);
			   fprintf(fpdiag,"      tiledmu crc %4x %4x\n",crc1,crc2);
			   fprintf(fpdiag,"   calculated crc %4x %4x\n",crcaa,crcb);
			   fflush(fpdiag);
			 }
		     crc(1,word);
		   }
		   ...
		   { crc(1,word);
		     crcaa = crca & 0x7fff;
                     if(sw(7) == 0) printf(" data word= %4i %4i %4x %4x %4x    %8x  %4x %4x\n",
