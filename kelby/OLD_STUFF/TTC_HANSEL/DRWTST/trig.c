...
char *envs[];
  { FILE *fp;
    ...
    int card,okcd[48],nall,ngood,nbad,dnum,ntry;
...
retry:
         ntry++;
         kkk=cardchk(kl);
         if(kkk != 1) kkk = cardchk(kl);  /* second try to find the card */
	 if(kkk == 1) 
         { card = kl;
...
/* now gets sums for mean calculation */  
	if(fdata[kk][ipk[kk]] < 100)
	 {trouble++;
	  if(trouble== 10) 
	  {printf("card %d error ipk=%d fdata[ipk]=%d\n",
	       card,ipk[kk],fdata[kk][ipk[kk]]);
	       gateok[card-1]=0;
	       noise[card-1]=0;
	       nlin[card-1]=0;
	       bb[card-1]=0;
	fwrite(&(gateok[card-1]),sizeof(gateok[card-1]),1,fpw);
    fwrite(&(noise[card-1]),sizeof(noise[card-1]),1,fpw);
    fwrite(&(nlin[card-1]),sizeof(nlin[card-1]),1,fpw); 
    fwrite(&(bb[card-1]), sizeof(bb[card-1]),1,fpw);
	  goto next;
	  }
          goto re2;
	 }
	...
	if(okcd[card-1] != 0) 
	  { printf(" card %d failed %d  gateok=%d  noise=%f  nlin=%f\n",
	      card,okcd[card-1],gateok[card-1],noise[card-1],nlin[card-1]);
            printf("        a=%f b=%f y=a+b*charge   ipk=%d pk=%d ntry=%d\n",
            aa[card-1],bb[card-1],ipk[kk],fdata[kk][ipk[kk]],ntry);
	    if(ntry<5) goto retry;
	    nbad++;
	  }
        nall++;
