...
int
nerror=0,ndcrc=0,nbnch=0,ngcrc=0,nctrl=0,nsdmu=0,ndigm=0,nstr1[16],nstr2[16],nperr[16];
...
static int icap;
int dskw1,dskw2,klog;
FILE *fpcfg,*fperr;
main(argc,argv,envs)
int argc;
char *argv[];
char *envs[];
  { int tim,i,l,k,kkk,lll,pipe,drawer,dig,kk,tube,kja,evkja,ir;
    int dac,alarm,rate;
    double charge,dacval,capacitor,r,rr;
    char xstring[80],ystring[80],string[80],err[80];
    double asum,axsum,axxsum,ped,dped,sig2,aval,yc;
    fperr = fopen("/home/user/kelby/DRWTST/digerr.log","w");
...
restrt:
	...
	for(i=0;i<16;i++)
	  { nstr1[i]=0;
        ...
        for(kja=0;kja<300;kja++) ranf(&r);  /* shift the start of the random numbers */
printf("setting up deskew clocks of digitizers\n");	  
	dskw1=0;
	dskw2=0;
	klog=0;
	ranf(&r);
	...
	ir=(int)rr;
	dskw1 = ir;
	printf(" r=%f  rr=%f  ir=%d  dskw1=%d\n",r,rr,ir,dskw1);
	ranf(&r);
        rr = (int)(240.0*r);
        ir=(int)rr;
        dskw2 = ir;
	printf(" r=%f  rr=%f  ir=%d  dskw2=%d\n",r,rr,ir,dskw2);
        for(kja=0;kja<8;kja++) 
	  { SetTimeDes1(digadr[kja],dskw1);
	    SetTimeDes2(digadr[kja],dskw2);
	  }
printf("starting loop\n");
 loop:
       ...
       if((event%25000) == 0) 
          {  printf("event=%d  nerror=%d  ndcrc=%d  nbnch=%d  nctrl=%d  ngcrc=%d  nsdmu=%d  ndigm=%d  dskw1=%d  desw2=%d  nloop=%d\n",
               event,nerror,ndcrc,nbnch,nctrl,ngcrc,nsdmu,ndigm,dskw1,dskw2,nloop);
	  }
       slink_read();
       if(sw(8) == 0) goto loop;
       if(sw(1) == 1) 
	 { Setcsr1(0x0028); /* disable triggers */
	   usleep(100000);
           clear_buffer();
	   printf("event=%d  nerror=%d  ndcrc=%d  nbnch=%d  nctrl=%d  ngcrc=%d  nsdmu=%d ndign=%d loop=%d\n",
                      event,nerror,ndcrc,nbnch,nctrl,ngcrc,nsdmu,ndigm,nloop);
           exit(0);
         }
          goto restrt;
   }  /* end of main */
...
slink_read()
	...
	int bnchok,gcrcok,dcrcok,ctrlok,sw7;
	int errsum,sessum,dessum,i;
	double epr,es1,es2;
	 	...
	 	{ kk = count/(nsamp+3);
		  ...
		  if(kklast == 15 && ii == 0) 
		 ...
		 else if(ii == 0) 
		   {
		     ...
		     bnch[hcnt] = bunch;
		     if(count == 1) b1 = bunch;
		     if(count!=1 && bunch!=b1) bnchok++;
		     hcnt++;
		     if(error != 0) errsum++;
                     if(sestr != 0) sessum++;
		     if(destr != 0) dessum++;
		     if(error != 0) nperr[hcnt]++;
                     if(sestr != 0) nstr1[hcnt]++;
		     if(destr != 0) nstr2[hcnt]++;
		   }
		 else if(ii>nsamp+1) 
		   {/* crc1 = (word>>16);
		     crc2 = word & 0xffff;
...
erxit:
			...
			if(klog==0)
			      ...
			      bnch[8],bnch[9],bnch[10],bnch[11],bnch[12],bnch[13],bnch[14],bnch[15]);
                            ...
                            fprintf(fperr,"bnch= %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x\n",
			      bnch[0],bnch[1],bnch[2],bnch[3],bnch[4],bnch[5],bnch[6],bnch[7],
			      bnch[8],bnch[9],bnch[10],bnch[11],bnch[12],bnch[13],bnch[14],bnch[15]);
                            if(dcrcok != 0) fprintf(fperr," dbad= %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d\n",
                              dbad[0],dbad[1],dbad[2],dbad[3],dbad[4],dbad[5],dbad[6],dbad[7],
                              dbad[8],dbad[8],dbad[10],dbad[11],dbad[12],dbad[13],dbad[14],dbad[15]);
			  }
		      } 
	            ...
	            if(gcrcok != 0) ngcrc++;
	            if(ctrlok != 0) nctrl++;
		    if(sessum>0 || dessum>0) nsdmu++;
		    if(errsum>0) ndigm++;		    
		    if(errsum>0 || sessum>0 || dessum>0) printf("errsum=%d  sessum=%d  dessum=%d\n",errsum,sessum,dessum);
                    if((errsum!=0 || sessum!=0 || dessum!=0) && klog==0)
                        {  fprintf(fperr,
			   "event=%d  nerror=%d  ndcrc=%d  nbnch=%d  nctrl=%d  ngcrc=%d  nsdmu=%d  ndigm=%d  dskw1=%d  dskw2=%d  nloop=%d\n",
                              event,nerror,ndcrc,nbnch,nctrl,ngcrc,nsdmu,ndigm,dskw1,dskw2,nloop);
                           printf(
			   "event=%d  nerror=%d  ndcrc=%d  nbnch=%d  nctrl=%d  ngcrc=%d  nsdmu=%d  ndigm=%d  dskw1=%d  dskw2=%d  nloop=%d\n",
                              event,nerror,ndcrc,nbnch,nctrl,ngcrc,nsdmu,ndigm,dskw1,dskw2,nloop);
                           for(i=0;i<16;i++)
	                     { epr = (double)nperr[i]/(double)event;    
	                       es1 = (double)nstr1[i]/(double)event;    
	                       es2 = (double)nstr2[i]/(double)event;
	                       fprintf(fperr," i=%d   nperr=%d   nstr1=%d   nstr2=%d   epr=%f   es1=%f   es2=%f\n",
	                         i,nperr[i],nstr1[i],nstr2[i],epr,es1,es2);
	                       printf(" i=%d   nperr=%d   nstr1=%d   nstr2=%d   epr=%f   es1=%f   es2=%f\n",
	                         i,nperr[i],nstr1[i],nstr2[i],epr,es1,es2);
	                     }
			   klog=1;
			   exit(0);
