...
char *argv[];
char *envs[];
  { int tim,i,l,k,kkk,pipe,drawer,dig,kk,tube;
    int dac;
...
slink_read()
        {
	unsigned long word;
	int type_of_word,n,tube,crc,ient;
	 	...
	 	{ if(word == 0x51115110)
		   {  if(sw(7) == 0) printf("\n\n\n\n\nstart control work %x\n",
		      ...
		      tube = -3;
		       ...
		       if(sw(7) == 0 || ient == 0) printf("end control word %x\n",
		          word);
		  else
		       printf("unknown control word %x  tube=%d\n",
		          word,tube);
		}      		
	 if (type_of_word != SLINK_CONTROL_WORD) 
		{sd.ldata =  word;
		 if(sd.bits.topbit == 1 && crc == 1) 
		   { parity = (word>>30) & 1;
		     ...
		     gain = (word>>12) & 0x7;
		     bunch = word & 0xfff;
		     if(sw(8) == 0) printf("header word %x\n",word);
		     if(sw(8) == 0) printf(" parity=%d  samples=%d  error=%d\n",
		        parity,samples,error);
		     if(sw(8) == 0) printf(" SEstr=%d   r_parity=%d   v_parity=%d\n",
		              sestr,r_parity,v_parity);
		     if(sw(8) == 0) printf(" mode=%d   gain=%d   bunch=%d\n",
		        mode,gain,bunch);
		     crc = 0;
		     n = 0;
		   }
		   ...
		   if(n < nsamp+1)
		    { if(tube<24 && sw(8) == 0)
		       printf(" data word %x  %i %i %4x %4x %4x  tubes=%d %d %d n=%d\n",
                       word,sd.bits.topbit,sd.bits.parity,
		     ...
		     { if(sw(8) == 0) printf(" crc word %x\n",word);
		       crc = 1;
		     } 
                   }
                }
        }		
	if(sw(8) == 0) printf("no data\n");
	return;
       }
