...
char *envs[];
  { 
    ...
    int tube;
    int dac;
    double charge,dacval,capacitor,fm,sg;
    ...
    nevmax=10;
    kk = 0;
    fp = fopen("fckh_all.dat","w");
  /*  printf("tube number\n ");
    fscanf(stdin,"%d",&tube);  */
    icap = 1;   /* small C high gain */
    id=1;
    ttcadr=0x1234;
    ttcvi_map(ttcadr);
   ...
   /* for now set up the digitizer ttcrx chips */
	...
	hstrst();
        ...
        hstlbl(0,"higain pedestal");

        hstset(1,5.0,0.0,50,1);
        hstlbl(1,"higain pedestal sigma");
...
/*  now get mean  */
   ...
   if(sw(7) == 1) printf(" high gain timer=%d sum=%f channel=%f peak=%f\n",
...
/*   show the plots */
   if(sw(7) == 1) printf(" max peak=%f  channel=%f  time=%f\n",pmaxl,cmaxl,tmaxl);
...
/* ---------------------- high gain calibration --------------------------*/
   ...
   sx2=0.0;
   dstep = 4;
   for (dac=24; dac<250; dac=dac+dstep)
     {set_dac(ttcadr,dac);
      usleep(1);
...
/*     display ? */
	...
	dped = 0.0;
        ...
        hstacc(1,dped,1.0);
        kpk=ipk[tube];
        if(fdata[tube][kpk]>1020 || charge>9.5) 
	  { if(sw(7) == 1) printf("overflow ped=%f  fdata=%d  charge=%f\n",
	         ped,fdata[tube][kpk],charge);  
	    goto lofit;
	  }
	...
	if(val<0.0) break;
        ...
        if(sev>0.0) 
          {val = sxev / sev;
           ...
           if(sw(7) == 1) printf("  high gain dac=%d charge=%f fmean=%f  sig=%f\n",
           dac,charge,val,sig);
          }
...
lofit:
   if(sw(7) == 1) printf(" s=%g  sx=%g  sy=%g  sxy=%g  sx2=%g\n",s,sx,sy,sxy,sx2);
   del = s*sx2 - sx*sx;
   a = (sx2*sy - sx*sxy) / del;
   b = (s*sxy - sx*sy) / del;
   al = a;
   bl = b;
   if(sw(7) == 1) printf(" higain fit results a=%f b=%f y=a+b*charge\n",a,b);
   for(k=0;k<icnt;k++)
    {x = crg[k];
     y = a + b*x;
     dely = y - fvl[k];
