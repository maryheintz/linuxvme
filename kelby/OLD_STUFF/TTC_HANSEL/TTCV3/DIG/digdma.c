#define SIZE_BUFFER 2000
#define WORD_BUFFER_SIZE 500
#define DMA_CHAIN_LENGTH 400
...
#include "SLINK.h"
#include "ttcvi.h"
unsigned long buffer[WORD_BUFFER_SIZE];
int ipackets,npackets,ipages,npages;
struct dmachain dma_chain[DMA_CHAIN_LENGTH];
char *pci_addr;
int page_size,packet_size,status;
...
char *envs[];
  { int tim,i,l,k,kkk,lll,pipe,drawer,dig,kk,tube;
    ...
    ttcadr=0x1234;
    ttcvi_map(ttcadr);
    Setcsr1(0x4028); /* disable triggers */
	...
	usleep(2);  
loop0:
    Setcsr1(0x402a); /* enable triggers */
 loop:
       inject();
       if(sw(4) == 0) 
         {  /* clear_buffer(); */ 
...
slink_setup()
	...
	printf("slink opened\n");
	/* get the physical address of the memory page of the buffer */
	if((npages = vmtopm((int)getpid(),dma_chain,(char *) buffer,
	SIZE_BUFFER)) <= 0) 
	  { printf("unable to get the chain of physical addresses of the buffer\n");
	    return(1);
	  }
	/* check the buffer is completely stored in one physical page of
	memory */
	if(npages>1) 
	  { printf(" the buffer has to be stored in only one physical memory page\n");
	    return(1);
	  }
	pci_addr = (char *)( dma_chain[0].address | 0x80000000);
	page_size = dma_chain[0].count; 
	printf(" pci_addr=%x  page_size=%d npages=%d\n",
	    pci_addr,page_size,npages); 
	return; 
	}

clear_buffer()
	{
	if(code = SSP_InitRead(dev, pci_addr, page_size, SLINK_ENABLE,0) != SLINK_OK)
	   { SLINK_PrintErrorMessage(code);
	     return(1);
	   }
...
slink_read()
	...
	int parity,samples,error,sestr,destr,r_parity,v_parity,
	   mode,gain,bunch,cc,kklast,i;
	int type_of_word,count,kk,ii,stop,hcnt,kkk;
	...
	kkk = packet_size/4;
	  ...
	  { kk = count/(nsamp+3);
	    ...
	    wordlast = buffer[i];
	    if(kklast == 15 && ii == 0) 
	       { if(sw(7) == 0) printf(" CRC_ERROR %x\n",word);
		 if(word != 0xffffffff) printf(" CRC_ERROR trouble %x\n",word);
               }
	    else if(kklast == 15 && ii == 1) 
	       { if(sw(7) == 0) printf(" final CRC %x    %d\n",word,kkk);
		 h1 = word & 0xffff;
		 h2 = (word>>16) & 0xffff;
		 if(h1 != h2) printf(" final CRC trouble %x\n",word);
	       }
	    else if(ii == 0) 
	       { parity = (word>>30) & 1;
		 samples = (word>>26) & 0xf;
		 ...
		 crc(0,word);
		 hcnt++;
	         if(sw(7) == 0) printf("%d header word %x\n",
		      hcnt,word);
	       }
	    else if(ii>nsamp+1) 
	       { if(sw(7) == 0) printf("      tileDMU  crc %x\n",
		       word);
		 crc(1,word);
               }
	    else
	       { crc(1,word);
                 if(sw(7) == 0) printf(" data word= %4i %4i %4x %4x %4x\n",
...
/* reset v3.0 ttcrx chips*/
 /*       if(mode == 1) 
	...
	SendLongCommand(adrttc,1,0xf,8);  /* set mode=0 NORMAL + RESET */	
	usleep(2);
	...
	SendLongCommand(adrttc,1,0xf,0);  /* set mode=0 */	
	usleep(2);
