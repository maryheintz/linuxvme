...
/* ====================== read out the 16 tileDMU chips ============== */
	...
	gain = (word>>12) & 0x7;
	bunch = word & 0xfff;
	printf("%d logain header word %x   bunch %d  %x %x\n"
	,block,word,bunch,crca,crcb);
/*	printf(" parity=%d  samples=%d  error=%d\n",
	...
	for(ii=0;ii<nsamp+1;ii++)
		...
		{ fdlow[3*kk+1][ii] = sd.bits.data1;
		  ...
		  fdlow[3*kk+3][ii] = sd.bits.data3;
                ...
                printf("lo data word= %4i %4i %4x %4x %4x",
	...
	gain = (word>>12) & 0x7;
	bunch = word & 0xfff;
	printf("%d higain header word=%x   bunch=%d  %x %x\n",
	   block,word,bunch,crca,crcb);
/*	printf(" parity=%d  samples=%d  error=%d\n",
	   parity,samples,error);
	printf(" SEstr=%d   r_parity=%d   v_parity=%d\n",
	...
	for(ii=0;ii<nsamp+1;ii++)
		...
		{ fdhi[3*kk+1][ii] = sd.bits.data1;
		  ...
		  fdhi[3*kk+3][ii] = sd.bits.data3;
                ...
                printf("hi data word= %4i %4i %4x %4x %4x",
	           sd.bits.topbit,sd.bits.parity,
		   sd.bits.data1,sd.bits.data2,
		   sd.bits.data3);
		printf("   %d  %d    %x   %x %x\n",kk,ii,word,crca,crcb);
	    ...
	    if(code != SLINK_OK) goto err;
	    if (type_of_word == SLINK_CONTROL_WORD) goto err;  
	        printf(" digcrc word=%x   %x\n",word,code);
            code = SSP_ReadOneWord(dev, &word, &type_of_word);
	    if(code != SLINK_OK) goto err;
	    if (type_of_word == SLINK_CONTROL_WORD) goto err;   
	        printf(" global crc word=%x   %x\n",word,code);
            code = SSP_ReadOneWord(dev, &word, &type_of_word);
	    if(code != SLINK_OK) goto err;
	    if (type_of_word != SLINK_CONTROL_WORD) goto err;   
...
/* reset v3.0 ttcrx chips*/
 /*       if(mode == 1) 
	...
	usleep(2);
        ...
        SendLongCommand(adrttc,1,1,nsamp); /* number of samples = 5 */
	usleep(2);	
	...
	SendLongCommand(adrttc,1,0xf,1);  /* set mode=1 calibration */	
	usleep(2);
