...
int id,ttcadr,itype,nall,ngood,nbad;
...
/*    isw[0]=0x1;
   ...
   for(gain=0;gain<6;gain++)
    {printf("starting gain=%d\n",gain);
     ...
     ii[gain]=0;
     dacset = max[gain];
     for(i=del[gain];i<dacset;i=i+del[gain])       
       { set_dac(ttcadr,i);
         usleep(100000);
	 for(card=1;card<49;card++)
	   { command = 0xc400 | card;
...
/*   printf("card=%d  gain=%d  dacset=%d  del=%d  imx=%d\n",
   ...
   for(card=1;card<49;card++)
    { dxyzro();
	 ...
	 bb[card][gain]=b;
         hstacc(gain,b,1.0);
         if(sw(7) == 1) printf(" card %d gain %d fit results a=%f b=%f volts=a+b*dac  ped=%f  sped=%f\n",
            card,gain,a,b,ped[card][gain],sped[card][gain]);
         if(a<-0.1 || a>0.1) ok[card]=6+gain;
   ...
   if(ok[card] != 0 && ok[card]<6 ) 
      printf(" card %d is BAD  ok=%d  b=%f\n",
               card,ok[card],bb[card][ok[card]]);    
