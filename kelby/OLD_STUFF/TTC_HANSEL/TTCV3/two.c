...
int digadr[8] = {0x10bb,0x10a9,0,0,0,0,0,0};
...
slink_read()
	...
	{if (type_of_word != SLINK_CONTROL_WORD) 
	 	{ kk = count/(nsamp+3);
		  ...
		  if(kklast == 15 && ii == 0) 
		   { if(sw8 == 0) printf(" dig crc %x\n",word);
		     if(word != 0xffffffff) 
		      { dcrcok++;
		        if(sw8==0) printf(" dig crc ERROR %x\n",word);
	              }
		   }
		   ...
		   { parity = (word>>30) & 1;
		     ...
		     if(count == 1) b1 = bunch;
		     if(count!=1 && bunch!=b1) 
		        { if(sw8==0) printf("bunch number error  block=%d  count=%d  bunch=%d  b1=%d\n",
		                    hcnt,count,bunch,b1);
		                    bnchok++;
					}
		     crc(0,word);
		     hcnt++;
		     if(hcnt>16)
			{if(sw8==0) printf("hcnt error count=%d\n",count);
                         slink_setup();
			 return;
			}
		 ...
		 if(cc == 1 && word != 0x51115110) 
		   { ctrlok++;
		     if(sw8==0) printf("control word= %x  cc %d  error %d  event %d\n",
...
erxit:
	...
	if(sw8==0) printf("no data   count=%d\n",count);
	return;
       }
