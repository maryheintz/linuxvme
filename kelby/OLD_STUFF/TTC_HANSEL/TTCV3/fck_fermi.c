...
/*   ttcadr = 0;  */
   ...
   for(k=0;k<49;k++)
     { can_tube(id,k);
         ...
         usleep(1);
         shrt=(i1<<8) | i2;
 	 stat=can_3in1set(id,shrt);       /* send to 3in1 card */
         usleep(1);
 	 stat=can_3in1set(id,0x9c00);       /* load back */
         usleep(1);
 	 stat=can_3in1get(id,&shrt);
	 if((int)shrt == itst) 
	   { printf(" card %d found\n",k);
	     tube = k;
	     goto found; }
...
found:
    ...
    timf=0;
        ...
        dotset(3,250.0,0.0,25.0,15.0,0,0);
        sprintf(xstring,"fine time setting");
    ...
    {double y;
	 ...
	 double a;
	   ...
	   {/* setup inhibit1 to set tp high */
	     ...
	     BPutLong(1,ttcadr,1,0x80,0x00);  /* load set tp_high into fifo1 */
	     /* set inhibit2 to drop tp low (timed broadcast mode */
	     SetBMode(2,0xa);
	     SetInhibitDelay(2,2000);
	     SetInhibitDuration(2,10);
	    /* BPutLong(2,ttcadr,1,0x80,0x01);  */ /* load set tp_low into fifo2 */
