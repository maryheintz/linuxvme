   ...
   usec=1;
    ...
    if(mode == 6)
       { printf(" select front panel output signals\n");
         ...
         printf(" 12 - fp_out[8..1] = trg[55..52]-trg[47..44]\n");
         ...
         printf(" 22 - setup AND - OR logic\n");
	 printf("       out[3] = out[4] = in[3] AND in[4]\n");
	 printf("       out[5] = out[6] = in[5] AND in[6]\n");
	 printf("       out[7] = out[8] = in[7] OR in[8]\n");
         printf(" 23 - FAN IN - FAN OUT logic\n");
	 printf("       out[3] = out[4] = out[5] = out[6] = out[7] = out[8] = \n");
	 printf("            in[3] OR in[4] OR in[5] OR in[6] OR in[7] OR in[8]\n");
...
/*  abcd 0=A  1=C  2=B  3=D  */
  ...
  sw(int i)
  { /* normally this routine reads the state of 8 switches
       on the front panel of a little VME module I have 
       returns 1=on  0=off   this dummy routine just
       returns 0 for all calls */
    int k,l,shift;
    /* k=0xff;  */
    k=0;
    shift = i-1;
