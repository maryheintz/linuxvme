...
//#include "old_header_files/s5933lib.h"
//#include "old_header_files/SLINK.h"
#include "ttcvi.h"
#include "vme_base.h"
...
char *envs[];
  { int tim,i,l,k,kkk,lll,pipe,drawer,dig,kk,tube;
    ...
    fpcfg = fopen("/home/kelby/SLINK_TEST/cfg.dat","r");
...
/*  label vertical axis  */
	...
	/*     BPutLong(1,ttcadr,1,0xc0,0x00); */ /* load set tp_high into fifo1 */
     ...
     inject()
       { static int entry=0;
         int c1,c2,f1,f2,swtst,i,i1,i2,i0time,i3time,kk;
	 double a;
         
	   a = sin(0.21);  
loop:      timset(20,0,30,0);
           stvme();  /* send pulses from the switch box to clear last event */
           usleep(1);
           swtst = latch();  /* wait for the latch bit to set */
           if(swtst == 0) 
             { usleep(100); 
/*    for(kk=1;kk<9;kk++) printf(" sw(%d) = %d\n",kk,sw(kk)); 
	     printf(" in inject loop\n");  */
            restim();
	    goto loop;
             }
...
slink_setup()
	...
	// Next line commented out by Mary
	//slink_para.position = 0;
	slink_para.occurence = 2;
	slink_para.data_width = SLINK_32BITS;
	slink_para.byte_swapping = SLINK_DISABLE;
	slink_para.timeout = 100;
	slink_para.start_word = 0x51115110;
	slink_para.stop_word = 0xfffffff0;
	slink_para.keep_bad_packets = SLINK_ENABLE;
        code = SLINK_SSPOpen(&slink_para, &dev);
	printf("slink opened code=%x\n",code);
/*	if ((code = SLINK_SSPOpen(&slink_para, &dev)) != SLINK_OK)
	{
	printf(" slink open error, error code = %d\n",code);
	// Next line commented by Mary because no SLINK_PrintErrorMessage in slink.h
...
clear_buffer()
    { unsigned long int word;
      ...
      int data_count = 0;
      if(sw(6) == 1)
        { printf(" clear_buffer dummy return\n");
	  return;
	}
      printf("before SSP_ReadOneWord\n");
      code = SSP_ReadOneWord(dev, &word, &type_of_word);
      printf("code=%x  word=%x\n",code,word);
...
slink_read()
	...
	int bnchok,gcrcok,dcrcok,ctrlok;
        ...
        if(sw(6)==1) 
	   { printf("slink_read dummy return\n");
	     return;
	   }
	cc = 0;
	count =0;
