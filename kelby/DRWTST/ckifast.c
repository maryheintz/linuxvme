...
char *argv[];
char *envs[];
  { int tim,i,l,k,kkk,lll,pipe,drawer,dig,kk,tube,kja,errx;
...
//    pio_enable();
    event++;
    if((event%50) == 0) 
             printf("event=%d  nerror=%d  ndcrc=%d  nbnch=%d  nctrl=%d  ngcrc=%d  %d   %d  %x  %x\n",
               event,nerror,ndcrc,nbnch,nctrl,ngcrc,need,used,wd1f,wd2f);
	...
	/*     BPutLong(1,ttcadr,1,0xc0,0x00); */ /* load set tp_high into fifo1 */
     ...
     inject()
       { static int entry=0,cnt;
         int c1,c2,f1,f2,swtst,i,i1,i2,i0time,i3time;
	 double a,b,c;
retry:     arbreset();  /* clear last event & ltch bit */
           waitkja(100);
