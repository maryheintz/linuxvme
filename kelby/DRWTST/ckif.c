...
unsigned int deskew2[9] = {60,60,60,60,60,60,60,60,60};
unsigned int iclksm[9] = {0,0,1,0,1,0,0,0,0};

int code;
static int nsamp;
int event=0,nerror=0,ndcrc=0,nbnch=0,ngcrc=0,nctrl=0,rstrt=0;
...
char *argv[];
char *envs[];
  { int tim,i,l,k,kkk,lll,dig,kk,tube,kja,errx;
    int dac,errors;
...
/*   printf("calling can_reset -- wait\n"); */
   ...
   printf(" good=%d    bad=%d     %d\n",good,bad,total);
    ...
    pio_reset();
    pio_status(&status);
    printf(" disabled status = %x\n",status);
    pio_enable();
    pio_status(&status);
    printf(" enabled status = %x\n",status);
...
/* ========== toss a few events to get started ============== */
    ...
    kja=pio_rdwd();
loop:
    if(sw(2) == 1) goto restart;
    event++;
    if((event%50) == 0) 
             printf("event=%d  nerror=%d  ndcrc=%d  nbnch=%d  nctrl=%d  ngcrc=%d %4x %4x  rstrt=%d\n",
               event,nerror,ndcrc,nbnch,nctrl,ngcrc,wd1f,wd2f,rstrt);
/*       BroadcastShortCommand(3);  */     /* BunchCrossing */
         inject();
...
digitizer_setup(int i)
	...
	usleep(2000);
        /* SendLongCommand(adrttc,0,0x3,0xa9); */ /* use bnch cntr */
        SendLongCommand(adrttc,0,0x3,0xa8);  /* use event cntr */
	usleep(0);  
