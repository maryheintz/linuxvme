...
printf(" fast 3\n");
        ...
        dotset(3,250.0,0.0,35.0,25.0,0,0);
        sprintf(xstring,"fine time setting");
...
chk_trig()
  { double asum,axsum,axxsum,ped,dped,sig2,aval,yc;
    ...
    factor2 = 2.0;  /* compensate for adder gain */
    fermi_setup();
    time1 = 0;
    time2 = 0;
    time3 = 0;
    ...
    tmaxl=0.0;
        ...
        dotset(5,200.0,0.0,4000.0,0.0,0,0);
        sprintf(xstring,"charge");
        sprintf(ystring,"trigger fermi peak");
        dotlbl(5,xstring,ystring);


        dotset(7,200.0,0.0,25.0,-25.0,0,0);
        sprintf(xstring,"charge");
...
/*-------------- test the trigger output switch -------------- */
   ...
   if(kFAIL == 0) fprintf(fp,"TRIGGER   noise=%6.2f slope=%6.2f nonlin=%6.2f   OK\n\n",
     noisel,slopel,dymaxl);
   if(kFAIL != 0) fprintf(fp,"TRIGGER   noise=%6.2f slope=%6.2f nonlin=%6.2f  BAD=%d\n\n",
     noisel,slopel,dymaxl,kFAIL);
   tok=1;
 ...
 { int isw[6],i1,i2,ll,i,mode,s1,s2,s3,s4,sw8,itr,ire,mse,tpe,lcal;
   ...
   if(sw(7) == 0)
    { sidev(1);
      dotmwr(6,0,5);
      dotmwr(6,6,11);
      if(fpsw(5) == 1) for(i=0;i<12;i++) dotwrt(i);
/*    sidev(0);
      dotmwr(6,0,5);  */
    }
}
