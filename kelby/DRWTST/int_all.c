...
unsigned int card;
   double slp[6] ={0.011,0.10,0.11,0.21,0.29,0.39};
...
for(card=1;card<49;card++) 
 { kkk = cardchk(card);
   if(kkk == 1 && sw(6)==1) printf("card %d found\n",card);
   if(kkk == 0) 
...
found:
   ...
   for(gain=0;gain<6;gain++)
    {if(sw(6)==1) printf("starting gain=%d\n",gain);
	 ...
	 if(sped[card][gain]/factor>lsped[gain]) ok[card] = 20+gain;
         if(sw(6)==1) printf("card=%d  gain=%d  ped=%f sped=%f  shrt=%x\n",
	...
	/* card=kard; */
	    command = 0xc400 | card;
	     ...
	     if(sw(6)==1) printf("card=%d dac=%d  volt=%f  ped=%f  vlt=%f shrt=%d\n",
...
/*   printf("card=%d  gain=%d  dacset=%d  del=%d  imx=%d\n",
   ...
   volt = factor * (double)shrt;
   v1[card] = volt;
   if(sw(6)==1) printf("card=%d intg_rd=0 itr=0 on volt=%f\n",
          card,volt);
   ...
   volt = factor * (double)shrt;
   v2[card] = volt;
   if(sw(6)==1) printf("card=%d intg_rd=0 itr=1 on volt=%f\n",
          card,volt);
   ...
   volt = factor * (double)shrt;
   v3[card] = volt;
   if(sw(6)==1) printf("card=%d intg_rd=1 itr=0 on volt=%f\n",
          card,volt);
...
/*	      if(mean/factor>lnonl[gain]) 
      ...
      if(sw(6)==1) printf(" %f  b=%f  b-expected=%f %f\n",a,b,slp[gain]-dslp[gain],
                      slp[gain]+dslp[gain]);    
	 } 
 /* card=kard; */
