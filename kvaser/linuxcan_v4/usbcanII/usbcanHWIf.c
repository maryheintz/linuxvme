...
VCanHWInterface hwIf = {
  ...
  usbcan_translate_and_send_message,
  0,  // proc_read
  0,  // close_all
  usbcan_get_time,
...
#endif
{
  ...
  while (dev->present) {
...
#else
    {
      ...
      default:
        DEBUGPRINT(2, (TXT ("issueBulkTransfer error (%d)\n"), ret));
        ret = -ESHUTDOWN;
        break;
      }
    }
  ...
  switch (cmd->head.cmdNo) {
    ...
    case CMD_RX_EXT_MESSAGE:
    {
      ...
      if (chan < (unsigned)vCard->nrChannels) {
        ...
        DEBUGPRINT(6, (TXT(" - vCanDispatchEvent id: %ld (ch:%d), time %ld\n"),
...
#else  // WinCE version
  {
    // Need access to all memory since a node can be on another thread's
    // stack, which we don't normally have access to.
    DWORD permissions = SetProcPermissions(0xffffffff);
    for (currHead = (&dev->replyWaitList)->next, tmpHead = currHead->next;
      ...
      currNode = (UsbcanWaitNode *)((char *)currHead - offsetof(UsbcanWaitNode,
                                                                list));
#endif
...
#else
void usbcan_send (OS_IF_TASK_QUEUE_HANDLE *work)
#endif
{
  unsigned int     i;
#if WIN32 || LINUX_VERSION_CODE < 0x020614
