...
#endif
{
    ...
    while ((tmp & DPRAM_INTERRUPT_ACTIVE) != 0) {
        writel(tmp | DPRAM_INTERRUPT_ACK, (void *)(hCd->baseAddr +
                                                   DPRAM_INTERRUPT_REG));
...
int pciCanInitHW (VCanCardData *vCard)
{
    ...
    if (!MemQSanityCheck(hCard)) {
        DEBUGPRINT(1, "<1> DPRAM communication failed - check hardware");
        vCard->cardPresent = 0;
        return -1;
    }
    ...
    timeOut = os_if_wait_event_interruptible_timeout(hCard->waitSwInfo, hCard->receivedSwInfo, 1000);
    if (!timeOut) {
    ...
    for(i = 0; addresses[i]; i++) {
        ...
        switch(i) {
            case 0:
                ...
                if (hCd->baseAddr == 0) {
                    DEBUGPRINT(1, "<1> Could not map address\n");
                    return 0;
                }
...
#endif
{
  ...
  VCanChanData   *chd = devChan->vChan;
    ...
    if (queuePos >= 0) {
        if (pciCanTransmitMessage(chd, &chd->txChanBuffer[queuePos]) == 0) {
...
//======================================================================
int pciCanWaitResponse (VCanCardData *vCard, heliosCmd *cmd, heliosCmd *replyPtr,
...
int pciCanInitOne (struct pci_dev *dev)
{
    ...
    request_irq(hCd->irq, pciCanInterrupt, SA_SHIRQ,
                "Kvaser PCIcanII", vCard);
#else
    request_irq(hCd->irq, pciCanInterrupt, IRQF_SHARED,
                "Kvaser PCIcanII", vCard);
#endif
...
int pciCanInitAllDevices (void)
{
    ...
    for (dev = NULL; found < PCICAN_MAX_DEV;) {
        dev = pci_find_device(KVASER_VENDOR, PC104PLUS_ID, dev);
        if (!dev) {
            // No more PCIcanII
            break;
        }
    ...
    for (dev = NULL; found < PCICAN_MAX_DEV;) {
        dev = pci_find_device(KVASER_VENDOR, PCICANXII_ID, dev);
        if (!dev) {
            // No more PCIcanII
            break;
        }
