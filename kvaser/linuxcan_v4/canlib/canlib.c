                        ...
                        CANOps **canOps, char officialName[])
{
  ...
  *canOps     = NULL;
...
#else
  // This function may have to be modified for cards other than USBcan/Leaf.
  // For now, we open all the CAN?: files and give the ones with dual channels
  // (according to a new IOCtl) the lowest numbers.
  HANDLE fd;
  int twoCount = 0;
  int cardNr;
  int channels;
  int device_channels[10];
  WCHAR filename[DEVICE_NAME_LEN + 1];

  memset(device_channels, 0, sizeof(device_channels));

  for(cardNr = 0; cardNr <= 9; cardNr++) { // The devices can have number 0..9
    wsprintf(filename, L"CAN%d:", cardNr);
    fd = CreateFile(filename, GENERIC_READ, 0, NULL, OPEN_EXISTING, 0, NULL);
    if (fd != INVALID_HANDLE_VALUE) {
      if (os_if_ioctl_read(fd, VCAN_IOC_GET_NRCHANNELS,
                           &channels, sizeof(channels)) == 0) {
        device_channels[cardNr] = channels;
        DEBUGPRINT((TXT("%s has %d channels\n"), filename, channels));
        switch (channels) {
        case 1:
          break;
        case 2:
          ...
          break;
        default:
          DEBUGPRINT((TXT("Unsupported number of channels!\n")));
          break;
        }
      } else {
        DEBUGPRINT((TXT("Error checking number of channels on %s %d\n"), filename, errno));
      }
      CloseHandle(fd);
    }
  }

  // If the device is not among the dual channel devices,
  // setup to start at the single channel ones instead.
  channels = 2;
  if (channel >= twoCount) {
    channels = 1;
    channel -= twoCount;
  ...
  for(cardNr = 0; cardNr <= 9; cardNr++) {
    if (device_channels[cardNr] == channels) {
      if (channel / channels == 0) {
        break;      // Correct card reached
      } else {
        channel -= channels;
      }
    }
  ...
  snprintf(devName, DEVICE_NAME_LEN, "CAN%d:", cardNr);
  *devChannel = channel % channels;
  *canOps = &vCanOps;
  sprintf(officialName, "KVASER USBcanII/Leaf channel %d/%d",
          cardNr, *devChannel);
...
CANLIB_API int __stdcall canClose (const CanHandle hnd)
{
  ...
  if (OS_IF_IS_CLOSE_ERROR(OS_IF_CLOSE_HANDLE(hData->fd)))
    return canERR_INVHANDLE;
#else
  canSetNotify(hnd, 0, 0);
  ret = OS_IF_CLOSE_HANDLE(hData->fd);

  os_if_delete_cond(&hData->notify_ack);
#endif
  OS_IF_FREE_MEM(hData);

#if !LINUX
  if (OS_IF_IS_CLOSE_ERROR(ret))
    return canERR_INVHANDLE;
  else
#endif
  return canOK;
...
#else
  // This function may have to be modified for cards other than USBcan/Leaf.
  // For now, we open all the CAN?: files and sum their respective number of
  // channels (according to a new IOCtl).
  HANDLE fd;
  int tmpCount = 0;
  int cardNr;
  int channels;
...
#else
//******************************************************
// This function associates a window handle with the CAN circuit.
// A WM__CANLIB message is sent to that window when certain events
 ...
 */
{
  ...
  if (notifyFlags == 0 || aHWnd == NULL) {
    ResetEvent(hData->notify_die);
#if !LINUX
    os_if_set_cond(&hData->notify_ack);
#endif
    WaitForSingleObject(hData->notify_die, INFINITE);
    CloseHandle(hData->notify_die);

    if (hData->notifyFd != 0)
      CloseHandle(hData->notifyFd);
    hData->notifyFd = 0;
    return canOK;
  }

  return hData->canOps->setNotify(hData, aHWnd, notifyFlags);
}
#endif
